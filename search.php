			...
			if(search && text && image) {
				if(search.style.display == 'none') {
					if (cachedResponse) {
						...
						xhr.send('ignition/lib/ajax/search_ajax.php?search=<?php echo urlencode($form->GetValue('search')); ?>');
					}
				} else {
                ...
                <div id="PageContent">
					...
					<table>
						<tr>
							...
							<td>
								<iframe style="float:left;" width="350" height="197" src="//www.youtube.com/embed/JeTfl0qFw5Q?rel=0&amp;wmode=transparent"></iframe>
							</td>
						</tr>
					</table>
					<div class="clear"></div>

					<?php /* Steve requested the Video to be added.
								...
								if((empty($sql) || (!empty($sql) && ($table->TotalRows > 0))) && (count($filter->SpecGroup) > 0)) {
									if((count($filter->SpecGroup) > 0) || (count($filter->Filter) > 0)) {
										...
										if(count($filter->Filter) > 0) {
											...
											echo sprintf('<div class="FilterOption FilterOptionRemove"><a href="%s%s"><span class="FilterOptionCross FilterOptionCrossRemove1">Remove All Filters</span></a></div>', $_SERVER['PHP_SELF'], param('search') ? sprintf('?search=%s', htmlentities(param('search'))) : '');

											foreach($filter->Filter as $filterItem) {
												$tempFilterStr = $filter->GetFilterQueryString($filterItem->ID);

												echo sprintf('<div class="FilterOption"><a href="%s%s"><span class="FilterOptionCross FilterOptionCross4">%s: %s</span></a></div>', $_SERVER['PHP_SELF'], (strlen($tempFilterStr) > 0) ? sprintf('?%s', $tempFilterStr) : '', $filterItem->Group->Name, $filterItem->GetUnitValue());
											}
										...
										if(count($filter->Category) > 0) {
											...
											foreach($filter->Category as $categoryItem) {
												...
												echo sprintf('<div class="FilterOption"><a href="%s%s"><span class="FilterOptionCross FilterOptionCross3">%s</span></a></div>', $_SERVER['PHP_SELF'], (strlen($tempFilterStr) > 0) ? sprintf('?%s', $tempFilterStr) : '', $categoryItem->Name);
											}
													...
													for($k=0; $k<$maxColumns; $k++) {
														if(isset($columnArr[$k][$j])) {
															if($disableFilters) {
																...
																echo sprintf('<td style="width:%s%%; vertical-align:top;"><a href="%s?%sfilter=%d">%s</a> (%d)</td>', number_format(100/$maxColumns, 2, '.', ''), $_SERVER['PHP_SELF'], (strlen($filter->FilterQueryString) > 0) ? sprintf('%s&amp;', $filter->FilterQueryString) : '', $columnArr[$k][$j]['Value_ID'], $columnArr[$k][$j]['UnitValue'], $columnArr[$k][$j]['Products']);
															}
														} else {
															echo '<td>&nbsp;</td>';
														}
