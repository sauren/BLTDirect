...
if(isset($_REQUEST['searchString']) && !empty($_REQUEST['searchString'])){
	...
	$search = new ProductSearch($_REQUEST['searchString'],'debit_product.php?pid=','INNER JOIN warehouse_stock w ON w.Product_ID = p.Product_ID');
}
			...
			<table cellspacing="0" class="catProducts">
				<tr>
					...
					while($search->Results->Row){
						...
						$productInStock = new DataQuery(sprintf("SELECT * FROM warehouse_stock WHERE Product_ID = %d", mysql_real_escape_string($search->Results->Row['Product_ID'])));
						if($productInStock->TotalRows > 0){

							$prod = new Product($search->Results->Row['Product_ID']);
							?>
								<tr>
									<?php if(!empty($prod->DefaultImage->Thumb->FileName)){ ?>
										<td><a href="./debit_product.php?pid=<?php echo $prod->ID; ?>&cat=<?php echo $category->ID; ?>"><img src="<?php if(!empty($prod->DefaultImage->Thumb->FileName)){ echo $GLOBALS['PRODUCT_IMAGES_DIR_WS']. $prod->DefaultImage->Thumb->FileName;} ?> border="0" width="<?php echo $prod->DefaultImage->Thumb->Width; ?>" height="<?php echo $prod->DefaultImage->Thumb->Height; ?>" alt="Click to View Product Information" /></a>
									<?php } else { ?>
										<td>&nbsp;</td>
									<?php } ?>
									...
									<td>
										<a href="./debit_product.php?pid=<?php echo $prod->ID; ?>&cat=<?php echo $category->ID; ?>" title="Click for More Information"><strong><?php echo $prod->Name; ?></strong><br />
									...
									<td><?php echo $prod->GetBuyIt(0,true,'debit_customise.php'); ?></td>
								</tr>
							<?php
						}
