...
function view($supplierId) {
    $page = new Page('Pending Debits', 'Listing all pending supplier debits.');
	$page->Display('header');

	$table = new DataTable('debits');
	$table->SetSQL(sprintf("SELECT *, DATE(Created_On) AS CreatedDate FROM debit WHERE Is_Paid='N' AND Supplier_ID=%d", $supplierId));
