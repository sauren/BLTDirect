...
$data = new DataQuery(sprintf("SELECT csi.Page_Request, csi.Created_On, csi.Customer_ID, cu.Contact_ID FROM customer_session_item_archive AS csi LEFT JOIN customer AS cu ON cu.Customer_ID=csi.Customer_ID WHERE csi.Session_ID=%d ORDER BY csi.Created_On ASC", mysql_real_escape_string($reportSession->ID)));
while($data->Row) {
	$sessionItems[] = array(
