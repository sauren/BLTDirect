...
if(!isset($_REQUEST['start']) || !isset($_REQUEST['end']) || !isset($_REQUEST['period'])) {
	$data = new DataQuery(sprintf("SELECT MIN(Created_On) AS Start_Date, MAX(Created_On) AS End_Date FROM customer_session"));
	if($data->TotalRows > 0) {
...
$form->AddField('showcustomers', 'Show Customers', 'checkbox', 'N', 'boolean', 1, 1);
...
for($i=0; $i<count($dates); $i++) {
	$data = new DataQuery(sprintf("SELECT COUNT(cs.Session_ID) AS Count FROM customer_session AS cs WHERE cs.Created_On>='%s' AND cs.Created_On<'%s' %s", mysql_real_escape_string($dates[$i]['Start']), mysql_real_escape_string($dates[$i]['End']), mysql_real_escape_string($sqlWhere)));
	$sessionCount = $data->Row['Count'];
	$data->Disconnect();

	$data = new DataQuery(sprintf("SELECT COUNT(cs.Session_ID) AS Count FROM customer_session AS cs INNER JOIN user_agent AS ua ON ua.User_Agent_ID=cs.User_Agent_ID AND ua.Is_Bot='Y' WHERE cs.Created_On>='%s' AND cs.Created_On<'%s' %s", mysql_real_escape_string($dates[$i]['Start']), mysql_real_escape_string($dates[$i]['End']), mysql_real_escape_string($sqlWhere)));
	$botSessionCount = $data->Row['Count'];
	$data->Disconnect();

	$data = new DataQuery(sprintf("SELECT COUNT(csi.Session_Item_ID) AS Count FROM customer_session_item AS csi %s WHERE csi.Created_On>='%s' AND csi.Created_On<'%s' %s", $sqlFrom, mysql_real_escape_string($dates[$i]['Start']), mysql_real_escape_string($dates[$i]['End']), mysql_real_escape_string($sqlWhere)));
	$sessionItemCount = $data->Row['Count'];
	$data->Disconnect();

	$data = new DataQuery(sprintf("SELECT COUNT(csi.Session_Item_ID) AS Count FROM customer_session_item AS csi %s INNER JOIN user_agent AS ua ON ua.User_Agent_ID=csi.User_Agent_ID AND ua.Is_Bot='Y' WHERE csi.Created_On>='%s' AND csi.Created_On<'%s' %s", $sqlFrom, mysql_real_escape_string($dates[$i]['Start']), mysql_real_escape_string($dates[$i]['End']), mysql_real_escape_string($sqlWhere)));
...
if($period == 'Y') {
	...
	$data = new DataQuery(sprintf("SELECT COUNT(cs.Session_ID) AS Count FROM customer_session AS cs WHERE cs.Created_On>='%s' AND cs.Created_On<'%s' %s", $summaryStartDate, $summaryEndDate, $sqlWhere));
	$sessionCount = $data->Row['Count'];
	$data->Disconnect();

	$data = new DataQuery(sprintf("SELECT COUNT(cs.Session_ID) AS Count FROM customer_session AS cs INNER JOIN user_agent AS ua ON ua.User_Agent_ID=cs.User_Agent_ID AND ua.Is_Bot='Y' WHERE cs.Created_On>='%s' AND cs.Created_On<'%s' %s", $summaryStartDate, $summaryEndDate, $sqlWhere));
	$botSessionCount = $data->Row['Count'];
	$data->Disconnect();

	$data = new DataQuery(sprintf("SELECT COUNT(csi.Session_Item_ID) AS Count FROM customer_session_item AS csi %s WHERE csi.Created_On>='%s' AND csi.Created_On<'%s' %s", $sqlFrom, $summaryStartDate, $summaryEndDate, $sqlWhere));
	$sessionItemCount = $data->Row['Count'];
	$data->Disconnect();

	$data = new DataQuery(sprintf("SELECT COUNT(csi.Session_Item_ID) AS Count FROM customer_session_item AS csi %s INNER JOIN user_agent AS ua ON ua.User_Agent_ID=csi.User_Agent_ID AND ua.Is_Bot='Y' WHERE csi.Created_On>='%s' AND csi.Created_On<'%s' %s",  $sqlFrom, $summaryStartDate, $summaryEndDate, $sqlWhere));
	$botSessionItemCount = $data->Row['Count'];
	$data->Disconnect();
...
if(($period != 'Y') && ($period != 'M')) {
	if($form->GetValue('showcustomers') == 'Y') {
		$data = new DataQuery(sprintf("SELECT csi.Customer_ID, c.Contact_ID, CONCAT_WS(' ', p.Name_Title, p.Name_First, p.Name_Initial, p.Name_Last) AS Name, COUNT(DISTINCT cs.Session_ID) AS Sessions, COUNT(DISTINCT csi2.Session_Item_ID) AS Session_Items, MAX(csi2.Created_On) AS Last_Seen_On FROM customer_session AS cs INNER JOIN customer_session_item AS csi ON cs.Session_ID=csi.Session_ID AND csi.Customer_ID>0 INNER JOIN customer_session_item AS csi2 ON cs.Session_ID=csi2.Session_ID INNER JOIN customer AS c ON c.Customer_ID=csi.Customer_ID INNER JOIN contact AS n ON n.Contact_ID=c.Contact_ID INNER JOIN person AS p ON p.Person_ID=n.Person_ID WHERE cs.Created_On>='%s' AND cs.Created_On<'%s' %s GROUP BY Customer_ID ORDER BY Last_Seen_On DESC", mysql_real_escape_string($start), mysql_real_escape_string($end), mysql_real_escape_string($sqlWhere)));
		while($data->Row) {
			$customers[] = array(
