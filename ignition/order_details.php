...
for($i=0; $i<count($order->Line); $i++) {
	...
	if(empty($order->Line[$i]->DespatchID)) {
		...
		if($order->Line[$i]->Product->DiscountLimit != '' && $discountVal > $order->Line[$i]->Product->DiscountLimit){
			$discountVal = $order->Line[$i]->Product->DiscountLimit;
		}
						...
						if(($order->Line[$i]->IsAssociative == 'Y') && ($order->Line[$i]->Product->ID == 0)) {
							...
							if((strlen($pid) > 0) && is_numeric($pid)) {
								...
								if($order->Line[$i]->Price < $order->Line[$i]->Product->PriceCurrent) {
									...
									if($order->Line[$i]->Product->DiscountLimit != '' && $discount > $order->Line[$i]->Product->DiscountLimit){
										$discount = $order->Line[$i]->Product->DiscountLimit;
									}
	    ...
	    <table cellspacing="0" class="orderDetails">
	      <tr>
	        ...
	        if($order->PaymentMethod->Reference != 'google') {
				...
				<?php if($isEditable && empty($order->Line[$i]->DespatchID)){ ?>
					<th nowrap="nowrap" style="padding-right: 5px;">Max Discount</td>
				<?php } ?>
	      ...
	      <tr <?php echo sprintf('style="background-color: %s;"', $backgroundColor); ?>>
			...
			if($order->PaymentMethod->Reference != 'google') {
				...
				<td nowrap="nowrap">
					...
					if($order->Line[$i]->Product->ID > 0) {
						if($isEditable && empty($order->Line[$i]->DespatchID)){
							...
							$discountPercentage = round((($order->Line[$i]->Discount / $order->Line[$i]->Quantity) / $order->Line[$i]->Product->PriceCurrent) * 100);
							if($order->Line[$i]->Product->DiscountLimit != '' && $discountPercentage > $order->Line[$i]->Product->DiscountLimit){
								echo sprintf('%d%%', $order->Line[$i]->Product->DiscountLimit);
							} else if($discountPercentage > 0){
								echo sprintf('%d%%', $discountPercentage);								
							}
						}
					}
					?>&nbsp;
				...
				<?php if($isEditable && empty($order->Line[$i]->DespatchID)){ ?>
					<?php if($order->Line[$i]->Product->DiscountLimit != '' && $order->Line[$i]->Product->DiscountLimit >= 0 && $order->Line[$i]->Product->DiscountLimit <= 100){ ?>
						<td align="center" nowrap="nowrap"><?php echo sprintf('%d%%', $order->Line[$i]->Product->DiscountLimit); ?></td>
					<?php } else { ?>
						<td>&nbsp;</td>
					<?php } ?>
				<?php } ?>
