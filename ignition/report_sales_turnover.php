...
function start() {
	...
	if(isset($_REQUEST['confirm'])) {
		if(($form->GetValue('range') != 'none') && (strlen($form->GetValue('range')) > 1)) {
			switch($form->GetValue('range')) {
				...
				$end = date('Y-m-d H:i:s');
				break;
				case 'thismonth': 	$start = date('Y-m-1 00:00:00');
				$end = date('Y-m-d H:i:s');
				break;
				...
				case 'lastmonth': 	$start = date('Y-m-1 00:00:00', mktime(0, 0, 0, date("m")-1, 1,  date("Y")));
				$end = date('Y-m-d H:i:s', mktime(0, 0, 0, date("m"), 1,  date("Y")));
				break;
				case 'last3months': $start = date('Y-m-1 00:00:00', mktime(0, 0, 0, date("m")-3, 1,  date("Y")));
				$end = date('Y-m-d H:i:s', mktime(0, 0, 0, date("m"), 1,  date("Y")));
				break;
				case 'last6months': $start = date('Y-m-1 00:00:00', mktime(0, 0, 0, date("m")-6, 1,  date("Y")));
				$end = date('Y-m-d H:i:s', mktime(0, 0, 0, date("m"), 1,  date("Y")));
				break;
    ...
    $page = new Page('Sales Turnover Report', '');
...
function report() {
    ...
    $form->AddField('confirm', 'Confirm', 'hidden', 'true', 'alpha', 4, 4);
	$form->AddField('start', 'Start Date', 'hidden', '0000-00-00 00:00:00', 'anything', 19, 19, true);
	$form->AddField('end', 'End Date', 'hidden', '0000-00-00 00:00:00', 'anything', 19, 19, true);
	
	$page = new Page('Sales Turnover Report: ' . cDatetime($form->GetValue('start'), 'longdatetime') . ' to ' . cDatetime($form->GetValue('end'), 'longdatetime'), '');
	$page->Display('header');
	?>
	
	<br />
	<h3>Turnover</h3>
	<p>Turnover stats per sales rep.</p>

	<table width="100%" border="0">
		<tr>
			<td align="left" style="border-bottom:1px solid #aaaaaa"><strong>User</strong></td>
			<td align="right" style="border-bottom:1px solid #aaaaaa"><strong>Turnover</strong></td>
		</tr>

		<?php
		$data = new DataQuery(sprintf("SELECT CONCAT_WS(' ', p.Name_First, p.Name_Last) AS User, SUM(Total-TotalTax) AS Turnover FROM orders AS o INNER JOIN users AS u ON u.User_ID=o.Owned_By INNER JOIN person AS p ON p.Person_ID=u.Person_ID WHERE o.Created_On BETWEEN '%s' AND '%s' AND o.Status NOT IN ('Cancelled', 'Incomplete', 'Unauthenticated') GROUP BY o.Owned_By", mysql_real_escape_string($form->GetValue('start')), mysql_real_escape_string($form->GetValue('end'))));
		if($data->TotalRows > 0) {
			while($data->Row) {
				?>

				<tr class="dataRow" onMouseOver="setClassName(this, 'dataRowOver');" onMouseOut="setClassName(this, 'dataRow');">
					<td align="left"><?php echo $data->Row['User']; ?></td>
					<td align="right">&pound; <?php echo number_format($data->Row['Turnover'], 2, '.', ','); ?></td>
				</tr>
