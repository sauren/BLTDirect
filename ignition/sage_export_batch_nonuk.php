...
function locked() {
	$page = new Page(sprintf('<a href="%s">Sage Export Batch (Non UK)</a> &gt; Integration Locked', $_SERVER['PHP_SELF']), 'Sage integration locked.');
	$page->Display('header');
...
function complete() {
	$page = new Page(sprintf('<a href="%s">Sage Export Batch (Non UK)</a> &gt; Integration Complete', $_SERVER['PHP_SELF']), 'Sage integration complete.');
...
function integrate() {
	...
	$data = new DataQuery(sprintf("SELECT Payment_Method_ID, Method FROM payment_method WHERE Reference NOT LIKE 'google'"));
	while($data->Row) {
		$paymentMethods[$data->Row['Payment_Method_ID']] = $data->Row['Method'];
	...
	while($data->Row) {
		...
		if(!isset($invoices[$data->Row['Date_Created']][$taxRate][$data->Row['Payment_Method_ID']])) {
			$invoices[$data->Row['Date_Created']][$taxRate][$data->Row['Payment_Method_ID']] = array();
		}

		$totals['Invoices'][$data->Row['Invoice_ID']] = $data->Row['Invoice_Total'];
		$invoices[$data->Row['Date_Created']][$taxRate][$data->Row['Payment_Method_ID']][] = $template;
                ...
                foreach($invoices['Invoices'] as $date=>$invoiceDate) {
					foreach($invoiceDate as $taxRate=>$invoiceMethod) {
						foreach($invoiceMethod as $paymentMethodId=>$invoiceData) {
	                        ...
	                        $template = new SageTemplateInvoiceContainer();
							$template->invoice->id = sprintf('batchinvoicenonuk-%s-%d-%s', str_replace('-', '', $date), $paymentMethodId, $taxRate);
							$template->invoice->accountReference = $form->GetValue('account');
							$template->invoice->currency = 'GBP';
							$template->invoice->invoiceAddress->company = sprintf('BLT Daily Sales Gateway Non UK (%s)', $paymentMethods[$paymentMethodId]);
                ...
                foreach($invoices['Credits'] as $date=>$invoiceDate) {
					foreach($invoiceDate as $taxRate=>$invoiceMethod) {
						foreach($invoiceMethod as $paymentMethodId=>$invoiceData) {
	                        ...
	                        $template = new SageTemplateInvoiceContainer();
							$template->invoice->id = sprintf('batchcreditnonuk-%s-%d-%s', str_replace('-', '', $date), $paymentMethodId, $taxRate);
							$template->invoice->accountReference = $form->GetValue('account');
							$template->invoice->currency = 'GBP';
							$template->invoice->invoiceAddress->company = sprintf('BLT Daily Sales Gateway Non UK (%s)', $paymentMethods[$paymentMethodId]);
							$template->invoice->invoiceDate = date('c', strtotime($date . ' 00:00:00'));
	...
	$page = new Page('Sage Export Batch (Non UK) (Post VAT Rise)', 'Integrate with Sage.');
		...
		foreach($invoices as $date=>$invoiceDate) {
			foreach($invoiceDate as $taxRate=>$invoiceMethod) {
				foreach($invoiceMethod as $paymentMethodId=>$invoiceData) {
		            ...
		            <tr>
						<td class="label">&nbsp;</td>
						<td class="input" nowrap="nowrap"><strong><u><?php echo $date; ?>:</u> <?php echo $paymentMethods[$paymentMethodId]; ?> (<?php echo $taxRate; ?>% VAT)</strong></td>
					</tr>
					<tr>
						<td class="label">&nbsp;</td>
