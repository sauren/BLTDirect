...
function start(){
	...
	$page = new Page('Failed Transactions Report', 'Please choose a start and end date for your report');
	...
	$window = new StandardWindow("Report on Failed Transactions.");
...
function report() {
	...
	$page = new Page('Failed Transactions Report: ' . cDatetime($form->GetValue('start'), 'longdatetime') . ' to ' . cDatetime($form->GetValue('end'), 'longdatetime'), '');
	$page->Display('header');
	?>

	<br />
	<h3>Failed Transactions</h3>
	<p>Transactions during the selected period which have been registered as failed.</p>

	<table width="100%" border="0">
		<tr>
			<td style="border-bottom: 1px solid #aaaaaa;"><strong>Transaction</strong></td>
			...
			<td style="border-bottom: 1px solid #aaaaaa;"><strong>Date Ordered</strong></td>
			<td style="border-bottom: 1px solid #aaaaaa;"><strong>Name</strong></td>
			<td style="border-bottom: 1px solid #aaaaaa;" align="right"><strong>Amount</strong></td>
		</tr>

		<?php
		$data = new DataQuery(sprintf("SELECT p.Payment_ID, p.Transaction_Type, p.Status_Detail, p.Amount, p.Created_On, o.Order_Prefix, o.Order_ID, CONCAT_WS(' ', o.Billing_First_Name, o.Billing_Last_Name) AS Order_Billing_Name, DATE(o.Created_On) AS Order_Date FROM payment AS p INNER JOIN orders AS o ON o.Order_ID=p.Order_ID WHERE p.Status LIKE 'FAIL' AND p.Created_On>='%s' AND p.Created_On<'%s' ORDER BY p.Payment_ID ASC", mysql_real_escape_string($form->GetValue('start')), mysql_real_escape_string($form->GetValue('end'))));
		if($data->TotalRows > 0) {
			while($data->Row) {
				?>

				<tr>
					<td><?php echo $data->Row['Payment_ID']; ?></td>
					<td><?php echo $data->Row['Transaction_Type']; ?></td>
					<td><?php echo $data->Row['Status_Detail']; ?></td>
					...
					<td><?php echo $data->Row['Order_Date']; ?></td>
					<td><?php echo $data->Row['Order_Billing_Name']; ?></td>
					<td align="right"><?php echo $data->Row['Amount']; ?></td>
				</tr>
