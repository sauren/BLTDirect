...
function start(){
	...
	$page = new Page('Products Created Report', 'Please choose a start and end date for your report');
...
function report() {
	...
	$page = new Page('Products Created Report: ' . cDatetime($form->GetValue('start'), 'longdatetime') . ' to ' . cDatetime($form->GetValue('end'), 'longdatetime'), '');
	$page->Display('header');
	?>
	...
	<table width="100%" border="0">
		<tr>
			...
			<td style="border-bottom:1px solid #aaaaaa;"><strong>Created Date</strong></td>
		</tr>

		<?php
		$data = new DataQuery(sprintf("SELECT * FROM product WHERE Created_On>='%s' AND Created_On<'%s' AND Is_Demo_Product='N' ORDER BY Created_On ASC", mysql_real_escape_string($form->GetValue('start')), mysql_real_escape_string($form->GetValue('end'))));
		if($data->TotalRows > 0) {
			while($data->Row) {
				?>
