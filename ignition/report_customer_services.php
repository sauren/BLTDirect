...
function start(){
	...
	$page = new Page('Customer Services Report', 'The number of Order notes added to Orders by category. Please choose a start and end date for your report');
	...
	$window = new StandardWindow("Report on Customer Services.");
...
function report($start, $end){
	...
	$page = new Page('Customer Service Report : ' . cDatetime($start, 'longdatetime') . ' to ' . cDatetime($end, 'longdatetime'), '');
	$page->Display('header');

	$data = new DataQuery(sprintf("select count(o.Order_Note_ID) as notes, o.Order_Note_Type_ID, ot.Type_Name from order_note as o, order_note_type as ot
									where o.Order_Note_Type_ID=ot.Order_Note_Type_ID and
									o.Created_On between '%s' and '%s' group by o.Order_Note_Type_ID order by ot.Type_Name", $start, $end));
	?>

	<table width="100%" border="0" >
	  <tr>
		<td style="border-bottom:1px solid #aaaaaa"><strong>Count</strong></td>
		<td style="border-bottom:1px solid #aaaaaa"><strong>Type</strong></td>
	  </tr>
	<?php
	while($data->Row){
		echo sprintf("<tr><td>%s x</td><td>%s</td></tr>", $data->Row['notes'] , $data->Row['Type_Name']);
		$data->Next();
	}
	$data->Disconnect();
	echo "</table>";
