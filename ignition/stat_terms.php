...
if(($displayStart != '0000-00-00 00:00:00') && ($displayEnd != '0000-00-00 00:00:00')) {
	$data = new DataQuery(sprintf("CREATE TEMPORARY TABLE temp_term SELECT COUNT(csi.Session_Item_ID) AS Page_Requests, cs.Session_ID, cs.Created_On, cs.User_Agent_ID FROM customer_session_archive AS cs LEFT JOIN customer_session_item_archive AS csi ON csi.Session_ID=cs.Session_ID WHERE cs.Created_On>='%s' AND cs.Created_On<'%s' AND cs.Referrer_Search_Term LIKE '%s' GROUP BY cs.Session_ID", mysql_real_escape_string($start), mysql_real_escape_string($end), mysql_real_escape_string($term)));
	$data->Disconnect();
