...
while($data->Row) {
	...
	$summaryData[$dateYear][$dateMonth]['DespatchesTotal'] = 0;
	$summaryData[$dateYear][$dateMonth]['OrdersTotal'] = 0;
	$summaryData[$dateYear][$dateMonth]['PurchasesTotal'] = 0;
	$summaryData[$dateYear][$dateMonth]['PurchasesCost'] = 0;
...
$data = new DataQuery(sprintf("SELECT COUNT(DISTINCT p.Purchase_ID) AS Purchases, SUM(pl.Cost*pl.Quantity) AS Cost, DATE_FORMAT(IF(p.Purchased_On>'0000-00-00 00:00:00', p.Purchased_On, p.Created_On), '%%Y-%%m') AS Purchased_Date FROM purchase AS p INNER JOIN purchase_line AS pl ON pl.Purchase_ID=p.Purchase_ID WHERE p.Order_ID=0 AND p.For_Branch GROUP BY Purchased_Date"));
while($data->Row) {
	$dateYear = substr($data->Row['Purchased_Date'], 0, 4);
	$dateMonth = (int) substr($data->Row['Purchased_Date'], 5, 2);

	if(isset($summaryData[$dateYear])) {
		if(isset($summaryData[$dateYear][$dateMonth])) {
			$summaryData[$dateYear][$dateMonth]['PurchasesTotal'] = $data->Row['Purchases'];
			$summaryData[$dateYear][$dateMonth]['PurchasesCost'] = $data->Row['Cost'];
		}
	}
...
$page = new Page('Purchase Costs Report');
$page->Display('header');
?>
...
<table width="100%" border="0">
	...
	for($y=$startYear; $y<=$endYear; $y++) {
		...
		<tr>
			...
			for($m=1; $m<=12; $m++) {
				if(isset($summaryData[$y][$m])) {
					...
					<td style="border-bottom: 1px dotted #ccc;" align="right">
						<small><span style="color: #999;">Drop Ship Cost:</span></small><br />&pound;<?php echo number_format($summaryData[$y][$m]['Cost'], 2, '.', ','); ?><br /><br />
						...
						<small><span style="color: #999;">Purchase Cost:</span></small><br />&pound;<?php echo number_format($summaryData[$y][$m]['PurchasesCost'], 2, '.', ','); ?><br /><br />
						<small><span style="color: #999;">Prs Tot:</span> <?php echo $summaryData[$y][$m]['PurchasesTotal']; ?>x</small><br />
					...
					$totalPurchaseCost += $summaryData[$y][$m]['PurchasesCost'];
					$totalPurchaseTotal += $summaryData[$y][$m]['PurchasesTotal'];
				} else {
			...
			<td style="border-bottom: 1px dotted #ccc;" align="right">
				<small><span style="color: #999;">Drop Ship Cost:</span></small><br />&pound;<?php echo number_format($totalCost, 2, '.', ','); ?><br /><br />
				...
				<small><span style="color: #999;">Purchase Cost:</span></small><br />&pound;<?php echo number_format($totalPurchaseCost, 2, '.', ','); ?><br /><br />
				<small><span style="color: #999;">Prs Tot:</span> <?php echo $totalPurchaseTotal; ?>x</small><br />
			</td>
		</tr>		
