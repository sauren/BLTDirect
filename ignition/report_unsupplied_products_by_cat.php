...
function start(){
	...
	$page = new Page('Unsupplied Products by Category Report', 'Please choose a category for your report');
...
$page->Display('header');
	...
	$window = new StandardWindow("Report on Unsupplied Products by Category.");
...
function report($cat,$sub){
	...
	//echo "RAAAH";exit;
	$page = new Page('Unsupplied Products by Category Report');
	$page->AddToHead('<script language="javascript" src="js/scw.js" type="text/javascript"></script>');
$page->Display('header');
	...
	?>
	<h3><br />
	Products Without A Supplier</h3>
	<p>Unsupplied products that belong to <?php echo ($cat == 0)?"the root ":"the ".$catName->Row['Category_Title'];$catName->Disconnect();echo ($sub)? " category and all of it's subfolders":" category";?>
	</p>
	<table width="100%" border="0">
	  ...
	  if($cat == 0){
	  	...
	  	$top25 = new DataQuery(sprintf("select p.* from product p INNER JOIN product_in_categories c ON p.Product_ID = c.Product_ID where p.Discontinued <>'Y' AND %s", $clientString));
	  }
	  while($top25->Row){
	  	//Get supplier details and price details;
	  	$supCheck = new DataQuery(sprintf("SELECT sp.* FROM supplier_product sp INNER JOIN supplier s ON s.Supplier_ID = sp.Supplier_ID WHERE Product_ID = %d",$top25->Row['Product_ID']));
