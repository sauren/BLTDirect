...
function view() {
	...
	$invoiceSql = sprintf("SELECT *, CONCAT_WS(' ', Invoice_First_Name, Invoice_Last_Name) AS Invoice_Name FROM invoice WHERE Integration_ID='' AND Is_Paid='N' AND (Invoice_Tax=0 OR Invoice_Country NOT LIKE 'United Kingdom')");
	...
	$page = new Page('Pending Invoices (Non UK or Non VAT)', 'Below is a list of all invoices currently outstanding.');
	$page->Display('header');
	...
	$data = new DataQuery(sprintf("SELECT COUNT(*) AS Counter FROM invoice WHERE Integration_ID='' AND Is_Paid='N'"));
	if($data->Row['Counter'] > 0) {
		...
		echo $window->OpenContent();
		echo $webForm->Open();
		echo $webForm->AddRow('', sprintf('<input type="button" name="print" value="print all" class="btn" onclick="popUrl(\'%s?action=printall\', 650, 500);">', $_SERVER['PHP_SELF']));
		echo $webForm->Close();
		echo $window->CloseContent();
		echo $window->Close();
	}
...
function printAll() {
	...
	$data = new DataQuery(sprintf("SELECT Invoice_ID FROM invoice WHERE Integration_ID='' AND Is_Paid='N' AND (Invoice_Tax=0 OR Invoice_Country NOT LIKE 'United Kingdom')"));
	if($data->TotalRows > 0) {
