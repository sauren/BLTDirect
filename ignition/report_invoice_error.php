...
function start() {
	...
	$page = new Page('Invoice Error Report', 'Please choose a start and end date for your report');
...
function report() {
	...
	$page = new Page('Invoice Error Report: ' . cDatetime($form->GetValue('start'), 'longdatetime') . ' to ' . cDatetime($form->GetValue('end'), 'longdatetime'), '');
	$page->Display('header');
	?>

	<br />
	<h3>Invoices</h3>
	<p>All invoices for the selected period.</p>
	...
	<table width="100%" border="0">
		<tr>
		    ...
		    <td style="border-bottom:1px solid #dddddd;"><strong>Organisation</strong></td>
		    <td style="border-bottom:1px solid #dddddd;" align="right"><strong>Amount</strong></td>
		</tr>

		<?php
		$data = new DataQuery(sprintf("SELECT i.Invoice_ID, i.Order_ID, i.Invoice_First_Name, i.Invoice_Last_Name, i.Invoice_Organisation, i.Invoice_Total, i.Created_On FROM invoice AS i INNER JOIN payment_method AS pm ON pm.Payment_Method_ID=i.Payment_Method_ID WHERE i.Created_On>='%s' AND i.Created_On<'%s' AND pm.Reference LIKE 'credit'", $form->GetValue('start'), $form->GetValue('end')));
		while($data->Row) {
			?>

			<tr>
				<td><?php echo $data->Row['Invoice_ID']; ?></td>
				<td><?php echo $data->Row['Order_ID']; ?></td>
				<td><?php echo $data->Row['Created_On']; ?></td>
				<td><?php echo trim(sprintf('%s %s', $data->Row['Invoice_First_Name'], $data->Row['Invoice_Lasst_Name'])); ?></td>
				<td><?php echo $data->Row['Invoice_Organisation']; ?></td>
	...
	<input type="button" class="btn" value="print all" onclick="popUrl('?action=printall&start=<?php echo $form->GetValue('start'); ?>&end=<?php echo $form->GetValue('end'); ?>', 800, 600);" />
...
function printAll() {
	...
	$data = new DataQuery(sprintf("SELECT i.Invoice_ID FROM invoice AS i INNER JOIN payment_method AS pm ON pm.Payment_Method_ID=i.Payment_Method_ID WHERE i.Created_On>='%s' AND i.Created_On<'%s' AND pm.Reference LIKE 'credit'", $form->GetValue('start'), $form->GetValue('end')));
	while($data->Row) {
		$invoice = new Invoice($data->Row['Invoice_ID']);
	
		$key = md5($invoice->GetInvoiceAddress());
	...
	foreach($invoices as $invoiceArray) {
		if(count($invoiceArray) > 0) {
			...
			echo $invoice->GetErrorDocument($form->GetValue('start'), $form->GetValue('end'));
