...
function start(){
	...
	$page = new Page('Products Report', 'Please choose a start and end date for your report');
	...
	$window = new StandardWindow("Report on Products.");
...
function report($start, $end, $filter = 'N') {
	...
	$page = new Page('Products Report : ' . cDatetime($start, 'longdatetime') . ' to ' . cDatetime($end, 'longdatetime'), '');
	$page->AddToHead('<script language="javascript" src="js/scw.js" type="text/javascript"></script>');
	$page->Display('header');
	...
	if($filter != 'N') {
		$filterSQL = sprintf(" AND Order_Prefix='%s'", $filter);
	}
	?>

	<h1>Product Report</h1>
	<p><?php echo cDatetime($start, 'longdatetime'); ?> - <?php echo cDatetime($end, 'longdatetime'); ?></p>
	...
	<table width="100%" border="0">
		<tr>
			...
			<td style="border-bottom:1px solid #aaaaaa;" align="right"><strong>Average Sold Price</strong></td>
		...
		$rank = 1;
		$totalPrice = 0;
		$data = new DataQuery(sprintf("SELECT SUM(ol.Quantity) AS Quantity, ol.Product_ID, ol.Product_Title, SUM(ol.Line_Total-ol.Line_Tax) AS Total, COUNT(DISTINCT o.Order_ID) AS Orders, COUNT(DISTINCT ws.Stock_ID) AS Stocked_Items FROM order_line AS ol LEFT JOIN warehouse_stock AS ws ON ws.Product_ID=ol.Product_ID LEFT JOIN warehouse AS w ON w.Warehouse_ID=ws.Warehouse_ID AND w.Type='B' INNER JOIN orders AS o ON ol.Order_ID=o.Order_ID WHERE o.Created_On BETWEEN '%s' AND '%s' AND ol.Line_Status<>'Cancelled' AND o.Status NOT IN ('Cancelled', 'Incomplete', 'Unauthenticated') GROUP BY ol.Product_ID ORDER BY Orders DESC", mysql_real_escape_string($start), mysql_real_escape_string($end)));
		while($data->Row) {
	  		$totalPrice += $data->Row['Total'];
			?>

			<tr <?php echo ($data->Row['Stocked_Items'] == 0) ? ' style="background-color: #ffc;"' : ''; ?>>
				...
				<td align="right">&pound;<?php echo number_format($data->Row['Total'] / $data->Row['Quantity'], 2, '.', ','); ?></td>
