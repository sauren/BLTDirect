...
function integrate() {
						...
						for($i=0; $i<count($sageTemplate->customers); $i++) {
							if($sageTemplate->customers[$i]->customer->id == $data->Row['Contact_ID']) {
								...
								if($data2->TotalRows > 0) {
									$tax = new TaxCalculator(0, $data->Row['Billing_Country_ID'], $data->Row['Billing_Region_ID'], $data2->Row['Tax_Class_ID']);

									if(!empty($tax->TaxCode) && is_numeric($tax->TaxCode)) {
										$taxCode = $tax->TaxCode;
									}
								}
								$data2->Disconnect();
	...
	$page = new Page('Sage Export (Post VAT Rise)', 'Integrate with Sage.');
	$page->Display('header');
