...
function view(){
	...
	if(isset($_REQUEST['confirm']) && strtolower($_REQUEST['confirm']) == "true"){
		if($form->Validate()){
			$sql = sprintf("SELECT cu.Customer_ID, c.Contact_ID, c.Is_Active, c.Is_Customer, c.Is_Supplier, p.Name_First, p.Name_Initial, p.Name_Last, o.Org_Name FROM contact AS c LEFT JOIN customer AS cu ON cu.Contact_ID=c.Contact_ID LEFT JOIN contact AS c2 ON c.Parent_Contact_ID=c2.Contact_ID LEFT JOIN person AS p ON c.Person_ID=p.Person_ID LEFT JOIN organisation AS o ON c2.Org_ID=o.Org_ID LEFT JOIN address AS a ON a.Address_ID=p.Address_ID WHERE c.Contact_Type='I' AND c.Account_Manager_ID=%d", mysql_real_escape_string($GLOBALS['SESSION_USER_ID']));

			if(strlen($form->GetValue('contactid')) > 0) {
				if($form->GetValue('excludecontactid') == 'N') {
					$sql .= sprintf(" AND c.Contact_ID=%d", mysql_real_escape_string($form->GetValue('contactid')));
				} else {
					$sql .= sprintf(" AND c.Contact_ID<>%d", mysql_real_escape_string($form->GetValue('contactid')));
				}
			}
			...
			if(strlen($form->GetValue('fname')) > 0) {
				if($form->GetValue('excludefname') == 'N') {
					...
					$sql .= sprintf(" AND p.Name_First NOT LIKE '%%%s%%'", mysql_real_escape_string($form->GetValue('fname')));
				}
			}
			...
			if(strlen($form->GetValue('lname')) > 0) {
				if($form->GetValue('excludelname') == 'N') {
					...
					$sql .= sprintf(" AND p.Name_Last NOT LIKE '%%%s%%'", mysql_real_escape_string($form->GetValue('lname')));
				}
			}
			...
			if(strlen($form->GetValue('org')) > 0) {
				if($form->GetValue('excludeorg') == 'N') {
					...
					$sql .= sprintf(" AND o.Org_Name NOT LIKE '%%%s%%'", mysql_real_escape_string($form->GetValue('org')));
				}
			}
			...
			if(strlen($form->GetValue('email')) > 0) {
				if($form->GetValue('excludeemail') == 'N') {
					...
					$sql .= sprintf(" AND p.Email NOT LIKE '%%%s%%'", mysql_real_escape_string($form->GetValue('email')));
				}
			}
			...
			if(strlen($form->GetValue('phone')) > 0) {
				if($form->GetValue('excludephone') == 'N') {
					...
					$sql .= sprintf(" AND (REPLACE(p.Phone_1, ' ', '') NOT LIKE REPLACE('%%%s%%', ' ', '') AND REPLACE(p.Phone_2, ' ', '') NOT LIKE REPLACE('%%%s%%', ' ', ''))", mysql_real_escape_string($form->GetValue('phone')), mysql_real_escape_string($form->GetValue('phone')));
				}
			}
			...
			if(strlen($form->GetValue('mobile')) > 0) {
				if($form->GetValue('excludemobile') == 'N') {
					...
					$sql .= sprintf(" AND REPLACE(p.Mobile, ' ', '') NOT LIKE REPLACE('%%%s%%', ' ', '')", mysql_real_escape_string($form->GetValue('mobile')));
				}
			}
			...
			if(strlen($form->GetValue('fax')) > 0) {
				if($form->GetValue('excludefax') == 'N') {
					...
					$sql .= sprintf(" AND REPLACE(p.Fax, ' ', '') NOT LIKE REPLACE('%%%s%%', ' ', '')", mysql_real_escape_string($form->GetValue('fax')));
				}
			}
			...
			if(strlen($form->GetValue('address1')) > 0) {
				if($form->GetValue('excludeaddress1') == 'N') {
					...
					$sql .= sprintf(" AND a.Address_Line_1 NOT LIKE '%%%s%%'", mysql_real_escape_string($form->GetValue('address1')));
				}
			}
			...
			if(strlen($form->GetValue('address2')) > 0) {
				if($form->GetValue('excludeaddress2') == 'N') {
					...
					$sql .= sprintf(" AND a.Address_Line_2 NOT LIKE '%%%s%%'", mysql_real_escape_string($form->GetValue('address2')));
				}
			}
			...
			if(strlen($form->GetValue('address3')) > 0) {
				if($form->GetValue('excludeaddress3') == 'N') {
					...
					$sql .= sprintf(" AND a.Address_Line_3 NOT LIKE '%%%s%%'", mysql_real_escape_string($form->GetValue('address3')));
				}
			}
			...
			if(strlen($form->GetValue('city')) > 0) {
				if($form->GetValue('excludecity') == 'N') {
					...
					$sql .= sprintf(" AND a.City NOT LIKE '%%%s%%'", mysql_real_escape_string($form->GetValue('city')));
				}
			}
			...
			if(strlen($form->GetValue('postcode')) > 0) {
				if($form->GetValue('excludepostcode') == 'N') {
					...
					$sql .= sprintf(" AND REPLACE(a.Zip, ' ', '') NOT LIKE REPLACE('%%%s%%', ' ', '')", mysql_real_escape_string($form->GetValue('postcode')));
				}
			}
	...
	$form2->AddField('action', 'Action', 'hidden', 'merge', 'alpha', 10, 10);
	...
	if(!empty($sql)){
		...
		while($table->Table->Row) {
			$form2->AddField('merge_'.$table->Table->Row['Contact_ID'], 'Merge contact', 'checkbox', 'N', 'boolean', 1, 1, false);
	...
	if(isset($_REQUEST['action']) && ($_REQUEST['action'] == 'merge') && isset($_REQUEST['confirm'])) {
		if(isset($_REQUEST['primary'])) {
			...
			for($i=0;$i<count($contacts);$i++) {
				if($form2->GetValue('merge_'.$contacts[$i]['Contact_ID']) == 'Y') {
					if(((is_numeric($contacts[$i]['Customer_ID']) && ($contacts[$i]['Customer_ID'] > 0)) ? 'Y' : 'N') != $primaryCustomer) {
						$form2->AddError('Merging contacts must be of the same type as the primary contact.');
						break;
					}
				}
			}
		...
		if($form2->Valid) {
			if(isset($_REQUEST['primary'])) {
				for($i=0;$i<count($contacts);$i++) {
					if($form2->GetValue('merge_'.$contacts[$i]['Contact_ID']) == 'Y') {
						if($_REQUEST['primary'] != $contacts[$i]['Contact_ID']) {
							if($primaryCustomer == 'Y') {

								$data = new DataQuery(sprintf("SELECT Customer_ID FROM customer WHERE Contact_ID=%d", mysql_real_escape_string($_REQUEST['primary'])));
								if($data->TotalRows > 0) {
									$data2 = new DataQuery(sprintf("UPDATE invoice SET Customer_ID=%d WHERE Customer_ID=%d", $data->Row['Customer_ID'], $contacts[$i]['Customer_ID']));
									$data2->Disconnect();

									$data2 = new DataQuery(sprintf("UPDATE quote SET Customer_ID=%d WHERE Customer_ID=%d", $data->Row['Customer_ID'], $contacts[$i]['Customer_ID']));
									$data2->Disconnect();

									$data2 = new DataQuery(sprintf("UPDATE orders SET Customer_ID=%d WHERE Customer_ID=%d", $data->Row['Customer_ID'], $contacts[$i]['Customer_ID']));
									$data2->Disconnect();

									$data2 = new DataQuery(sprintf("UPDATE proforma SET Customer_ID=%d WHERE Customer_ID=%d", $data->Row['Customer_ID'], $contacts[$i]['Customer_ID']));
									$data2->Disconnect();

									$data2 = new DataQuery(sprintf("UPDATE `return` SET Customer_ID=%d WHERE Customer_ID=%d", $data->Row['Customer_ID'], $contacts[$i]['Customer_ID']));
									$data2->Disconnect();

									$data2 = new DataQuery(sprintf("UPDATE enquiry SET Customer_ID=%d WHERE Customer_ID=%d", $data->Row['Customer_ID'], $contacts[$i]['Customer_ID']));
									$data2->Disconnect();

									$data2 = new DataQuery(sprintf("UPDATE customer_contact SET Customer_ID=%d WHERE Customer_ID=%d", $data->Row['Customer_ID'], $contacts[$i]['Customer_ID']));
									$data2->Disconnect();

									$data2 = new DataQuery(sprintf("UPDATE customer_product SET Customer_ID=%d WHERE Customer_ID=%d", $data->Row['Customer_ID'], $contacts[$i]['Customer_ID']));
									$data2->Disconnect();

									$data2 = new DataQuery(sprintf("UPDATE customer_session SET Customer_ID=%d WHERE Customer_ID=%d", $data->Row['Customer_ID'], $contacts[$i]['Customer_ID']));
									$data2->Disconnect();

									$data2 = new DataQuery(sprintf("UPDATE customer_session SET Affiliate_ID=%d WHERE Affiliate_ID=%d", $data->Row['Customer_ID'], $contacts[$i]['Customer_ID']));
									$data2->Disconnect();

									$data2 = new DataQuery(sprintf("UPDATE customer_session_item SET Customer_ID=%d WHERE Customer_ID=%d", $data->Row['Customer_ID'], $contacts[$i]['Customer_ID']));
									$data2->Disconnect();

									$data2 = new DataQuery(sprintf("UPDATE discount_banding_basket SET Customer_ID=%d WHERE Customer_ID=%d", $data->Row['Customer_ID'], $contacts[$i]['Customer_ID']));
									$data2->Disconnect();

									$data2 = new DataQuery(sprintf("UPDATE discount_customer SET Customer_ID=%d WHERE Customer_ID=%d", $data->Row['Customer_ID'], $contacts[$i]['Customer_ID']));
									$data2->Disconnect();
								}
								$data->Disconnect();
							}

							$data = new DataQuery(sprintf("UPDATE contact_group_assoc SET Contact_ID=%d WHERE Contact_ID=%d", mysql_real_escape_string($_REQUEST['primary']), mysql_real_escape_string($contacts[$i]['Contact_ID'])));
							$data->Disconnect();

							$data = new DataQuery(sprintf("UPDATE campaign_contact SET Contact_ID=%d WHERE Contact_ID=%d", mysql_real_escape_string($_REQUEST['primary']), mysql_real_escape_string($contacts[$i]['Contact_ID'])));
							$data->Disconnect();

							$data = new DataQuery(sprintf("UPDATE supplier SET Contact_ID=%d WHERE Contact_ID=%d", mysql_real_escape_string($_REQUEST['primary']), mysql_real_escape_string($contacts[$i]['Contact_ID'])));
							$data->Disconnect();
			...
			foreach($_REQUEST as $key=>$value) {
				if(($key != 'primary') && ($key != 'action') && (substr($key, 0, (strlen($key) >= 5) ? 5 : strlen($key)) != 'merge')) {
			...
			redirect(sprintf("Location: %s%s", $_SERVER['PHP_SELF'], $sqlStr));
		}
	}
				...
				if(count($contacts) > 0) {
					...
					?>
						...
						<tr class="dataRow" onMouseOver="setClassName(this, 'dataRowOver');" onMouseOut="setClassName(this, 'dataRow');">
							<td align="center" width="16"><?php echo $form2->GetHTML('primary', $i+1); ?></td>
							<td align="center" width="16"><?php echo $form2->GetHTML('merge_'.$contacts[$i]['Contact_ID']); ?></td>
