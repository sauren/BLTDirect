...
function start(){
	...
	$page = new Page('Busy Period Report', 'Please choose a start and end date for your report');
	...
	$form->AddField('prefix', 'Order Type', 'select', 'T', 'alpha_numeric', 0, 32);
...
$page->Display('header');
	...
	$window = new StandardWindow("Report on Busy Periods.");
...
function report($start, $end, $prefix){
	...
	$page = new Page('Busy Period Report : ' . cDatetime($start, 'longdatetime') . ' to ' . cDatetime($end, 'longdatetime'), '');
	$page->AddToHead('<script language="javascript" src="js/scw.js" type="text/javascript"></script>');
$page->Display('header');
	?>
	<br />
	<h3>Busy Periods</h3>
	<p>Listing busiest hours for the given period.</p>

	<?php
	$orders = array();
	$total = 0;

	$data = new DataQuery(sprintf("SELECT Created_On FROM orders WHERE Created_On BETWEEN '%s' AND '%s' AND Order_Prefix='%s'", $start, $end, mysql_real_escape_string($prefix)));
	$total = $data->TotalRows;
	while($data->Row) {
		$key = (int) substr($data->Row['Created_On'], 11, 2);
	...
	<table width="100%" border="0">
	  <tr>
		<td style="border-bottom:1px solid #aaaaaa" align="left"><strong>Hour</strong></td>
		<td style="border-bottom:1px solid #aaaaaa"><strong>Order Count</strong></td>
	  ...
	  for($i=0;$i<24;$i++) {
	  	...
	  	<tr class="dataRow" onMouseOver="setClassName(this, 'dataRowOver');" onMouseOut="setClassName(this, 'dataRow');">
			<td><?php echo (($i > 12) ? ($i-12).'pm' : $i.'am') . ' - ' . (($j > 12) ? ($j-12).'pm' : $j.'am'); ?></td>
			<td><?php echo isset($orders[$i]) ? $orders[$i] : 0; ?></td>
			<td><?php echo number_format(isset($orders[$i]) ? ($orders[$i] / $total) * 100 : 0, 2, '.', ''); ?>%</td>
		</tr>
