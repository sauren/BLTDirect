...
function start(){
	...
	// type of report
	$form->AddField('type', 'Type of report', 'select', 'cards', 'alpha_numeric', 0, 32);
	$form->AddOption('type', 'all', 'All Card Transaction');
	$form->AddOption('type', 'cards', 'Credit Cards');
	$form->AddOption('type', 'cardsMoto', 'Credit Cards (MOTO)');
	$form->AddOption('type', 'amex', 'AMEX');
	$form->AddOption('type', 'amexMoto', 'AMEX (MOTO)');
	...
	if(isset($_REQUEST['confirm']) && strtolower($_REQUEST['confirm']) == "true"){
		...
		if(($form->GetValue('range') != 'none') && (strlen($form->GetValue('range')) > 1)) {
			...
			if($form->Validate()){
				report(sprintf('%s-%s-%s 00:00:00', 
							substr($form->GetValue('start'), 6, 4), 
							substr($form->GetValue('start'), 3, 2), 
							substr($form->GetValue('start'), 0, 2)), 
						(strlen($form->GetValue('end')) > 0) ? 
							sprintf('%s-%s-%s 00:00:00', 
								substr($form->GetValue('end'), 6, 4), 
								substr($form->GetValue('end'), 3, 2), 
								substr($form->GetValue('end'), 0, 2)) : date('Y-m-d H:i:s', mktime(0, 0, 0, date('m', strtotime(sprintf('%s-%s-%s 00:00:00', substr($form->GetValue('start'), 6, 4), substr($form->GetValue('start'), 3, 2), substr($form->GetValue('start'), 0, 2)))), date('d', strtotime(sprintf('%s-%s-%s 00:00:00', substr($form->GetValue('start'), 6, 4), substr($form->GetValue('start'), 3, 2), substr($form->GetValue('start'), 0, 2)))) + 1, date('Y', strtotime(sprintf('%s-%s-%s 00:00:00', substr($form->GetValue('start'), 6, 4), substr($form->GetValue('start'), 3, 2), substr($form->GetValue('start'), 0, 2)))))), $type);
				exit;
			}
		}
	}
...
/*
	reportSql()
	gets the report's sql in a single function rather than copy pasting
	it all over the place
*/
function reportSql($start, $end, $type, $conditions=array()){
	$sql = buildSql("select p.Payment_ID, p.Amount, p.Order_ID, o.*, i.Invoice_Net, i.Invoice_Shipping, i.Invoice_Discount, i.Invoice_Tax, i.Invoice_Total from payment as p 
						...
						where (p.Transaction_Type='PAYMENT' OR p.Transaction_Type='AUTHORISE') 
							and p.Status='OK'
							and p.Created_On between '%s' and '%s' 
							AND p.Security_Key NOT LIKE '%%GoogleCheckout%%'
							%s
						order by p.Created_On asc",
						$start, $end, $type, $conditions);
	return $sql;
}

function buildSql($sql, $start, $end, $type, $conditions=array()){
	...
	if($type != 'all'){
		$whereConditions[] = (stristr($type, 'amex'))? "AND o.Card_Type LIKE 'American Express'" : "AND o.Card_Type NOT LIKE 'American Express'";
		if(stristr($type, 'moto')){
			$whereConditions[] = "AND o.Order_Prefix='T'";
		} else {
			$whereConditions[] = "AND o.Order_Prefix!='T'";
		}
	}
	
	$whereConditions = array_merge($whereConditions, $conditions);

	$whereConditions = implode(' ', $whereConditions);
	$sql = sprintf($sql,
...
function report($start, $end, $type){
	...
	$reportType = array(
		'all' => 'All Card Transactions',
		'cards' => 'Credit Cards',
		'cardsMoto' => 'Credit Cards (MOTO)',
		'amex' => 'AMEX',
		'amexMoto' => 'AMEX (MOTO)'
	);
	...
	$page = new Page("VPS Payments Report for {$typeName}: " . cDatetime($start, 'longdatetime') . ' to ' . cDatetime($end, 'longdatetime'), 'This report is only applicable if you are using a payment gateway to take credit card payments automatically.');
	$page->Display('header');
	?>
	...
	$sql = reportSql($start, $end, $type, array(
	...
	$sql = buildSql("select p.Payment_ID, p.Amount, p.Order_ID, o.* from payment as p 
					inner join orders as o on p.Order_ID=o.Order_ID 
					where p.Transaction_Type='REFUND' 
						...
						AND o.TotalTax>0 
						%s
					order by p.Created_On asc", $start, $end, $type);
	...
	$sql = buildSql("select p.Payment_ID, p.Amount, p.Order_ID, o.* from payment as p 
						inner join orders as o on p.Order_ID=o.Order_ID 
						INNER JOIN card_type AS ct ON ct.Card_Type_ID=o.Card_Payment_Method
						where p.Transaction_Type='PREAUTH' 
							and p.Status='OK' 
							and p.Created_On between '%s' and '%s'
							%s
						order by p.Created_On asc", $start, $end, $type);
	...
	<?php
	// get all payments taken over time period
	$sql = buildSql("select p.Transaction_Type, p.Status, count(Payment_ID) as failures from payment as p 
					INNER JOIN orders AS o ON o.Order_ID=p.Order_ID 
					where p.Status!='OK' 
						and p.Created_On between '%s' and '%s'
						%s
					group by p.Transaction_Type, p.Status 
					order by p.Created_On asc", $start, $end, $type);
	$data = new DataQuery($sql);
	while($data->Row){
