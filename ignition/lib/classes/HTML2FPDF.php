...
require_once($GLOBALS["DIR_WS_ADMIN"] . 'lib/classes/FPDF.php');
require_once($GLOBALS["DIR_WS_ADMIN"] . 'lib/common/htmltoolkit.php');
...
class HTML2FPDF extends FPDF
{
	...
	function OpenTag($tag,$attr)
	{
		...
		switch($tag){
			...
			case 'TD':
				...
				//AZEXIS HACK
				$this->cell[$this->row][$this->col]['w'] -= ($this->cell[$this->row][$this->col]['w']/$this->table['w'])*$this->rMargin;
				// END HACK
			...
			case 'IMG':
				...
				if(isset($attr['SRC']))
				{
					...
					if ($this->tablestart)
					{
						...
						//AZEXIS HACK
						$this->cell[$this->row][$this->col]['w'] -= ((($this->cell[$this->row][$this->col]['w']-3)/$this->table['w'])*$this->rMargin) + 3;
						// END HACK
					}
				}
				break;
	...
	function printlistbuffer()
	{
		...
		foreach($this->listitem as $item)
		{
			...
			$type .= ' ';
			$custom_width = $this->GetStringWidth($type)+$this->cMargin*2;
			//Output bullet
			$this->Cell($custom_width,5,$type,'','','L');
	...
	function ReadMetaTags($html)
	{
		...
		$this->SetCreator("Azexis: http://www.azexis.com");
	}
