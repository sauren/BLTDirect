...
* © Copyright 2005 Richard Heyes
	...
	modified for looking for loans
*/

require_once($GLOBALS["DIR_WS_ADMIN"] . "lib/classes/mimePart.php");
...
class htmlMimeMail5
{
    ...
    */
    function htmlMimeMail5()
    {
        ...
        */
        ...
        $this->return_path = $GLOBALS['EMAIL_RETURN'];
        ...
        */
        ...
        $this->smtp_params['host'] = $GLOBALS['SMTP_HOST'];
    ...
    */ function setHTMLEncoding($encoding)
    {
    ...
    */ function setReturnPath($return_path)
	...
	function setReceipt($readReceipt)
    {
        $this->headers['Disposition-Notification-To'] = $readReceipt;
    }
    ...
    * @return mixed
    */
    function send($recipients, $type = 'mail', $forceBuild=false)
    {
    	if(!$GLOBALS['EMAIL_LIVE']) {
    		$recipients = array($GLOBALS['EMAIL_TECHNICAL']);
		}
...
class attachment
{
    ...
    */ function attachment($data, $name, $contentType, $encoding)
    {
        $this->data        = $data;
        $this->name        = $name;
...
class fileAttachment extends attachment
{
    ...
    */ function fileAttachment($filename, $contentType = 'application/octet-stream', $encoding = null)
    {
        $encoding = is_null($encoding) ? new Base64Encoding() : $encoding;

        parent::attachment(file_get_contents($filename), basename($filename), $contentType, $encoding);
    }
}
...
class stringAttachment extends attachment
{
    ...
    * @param string $contentType Content type of file
    * @param string $encoding    What encoding to use
    */ function stringAttachment($data, $name = '', $contentType = 'application/octet-stream', $encoding = null)
    {
        $encoding = is_null($encoding) ? new Base64Encoding() : $encoding;

        parent::attachment($data, $name, $contentType, $encoding);
    }
}
