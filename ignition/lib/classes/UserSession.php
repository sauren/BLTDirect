...
class UserSession {
	...
	function Create(){
		...
		new DataQuery(sprintf("INSERT INTO sessions (Session_ID, Created_On, User_ID, Last_Page, Referrer) VALUES ('%s', NOW(), %d, '%s', '%s')", mysql_real_escape_string($this->ID), mysql_real_escape_string($this->User->ID), mysql_real_escape_string(isset($_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : ''),mysql_real_escape_string(isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : '')));
	}

	function Update(){
		Session::UpdateUserSession($this->User->ID, $this->LastPage, $this->ID);
	}
	...
	function Get($id=NULL){
		...
		if($data->TotalRows > 0){
			...
			$this->IsLoggedIn = (empty($this->User->ID)) ? false : true;
			$data->Disconnect();
			return true;
		}
	...
	function Login($username = null, $password = null){
		$data = new DataQuery(sprintf("SELECT User_ID, User_Password FROM users WHERE User_Name LIKE '%s' AND Is_Active='Y' AND IsLocked='N'", mysql_real_escape_string($username)));
		if($data->TotalRows > 0) {
			if($data->Row['User_Password'] != md5($password)) {
				$user = new User($data->Row['User_ID']);
				$user->FailedLogins++;
				$user->Update();
				$data->Disconnect();
				return false;
	...
	function CreateQuickLoginUrl($email, $password) {
		$data = array($email=>$password);
		$data = serialize($data);
	    $cipher = new Cipher($data);
		$cipher->Encrypt();
		return "https://www.bltdirect.com/portal/teleprompt/login.php?action=auto&key=" . base64_encode($cipher->Value);
	}
}
