		...
		*******************************************************************************/
		function FPDF($orientation='P',$unit='mm',$format='A4')
		{
			...
			if(is_string($format))
			{
				...
				elseif($format=='label')	$format=array(620,310);
		...
		function AddFont($family,$style='',$file='')
		{
			...
			require_once($GLOBALS["DIR_WS_ADMIN"] . 'lib/classes/font/'.$file);
		...
		function WriteFlowingBlock( $s , $outofblock = false )
		{
			...
			// START AZEXIS HACK, calculate width for text based on width of page, units, and left and right margins.
			//$maxWidth = ($this->w - $this->lMargin - ($this->rMargin * 1)) * $this->k;
			// END AZEXIS HACK
...
*/
		...
		function Output($name='',$dest='', $makeunique = false, $outputDirectory = '')
		{
			...
			switch($dest)
			{
				...
				case 'F':
					//Save to local file
					if($makeunique) {
						if(file_exists($outputDirectory.$name)) {
							$i = 0;
							$extension = substr($name, strrpos($name, '.')+1);
							$newName = substr($name, 0, strrpos($name, '.'));
							while(true) {
								$i++;
								if(!file_exists($outputDirectory.$newName.'_'.$i.'.'.$extension)) {
									$name = $newName.'_'.$i.'.'.$extension;
									break;
								}
							}
						}
					...
					$f=fopen($outputDirectory.$name,'wb');
		...
		*******************************************************************************/
		...
		function _parsegif($file) //EDITEI - GIF support is now included
		{
			...
			require_once($GLOBALS["DIR_WS_ADMIN"] . 'lib/common/gif.php');
