...
?>
								...
								if((count($filter->SpecGroup) > 0) || (count($filter->Filter) > 0)) {
									...
									if(count($filter->Filter) > 0) {
										...
										echo sprintf('<div class="FilterOption FilterOptionRemove"><a href="%s%s"><div class="FilterOptionCross FilterOptionCrossRemove1">Remove All Filters</div></a></div>', $searchPage, isset($_REQUEST['search']) ? sprintf('?search=%s', htmlentities($_REQUEST['search'])) : '');

										foreach($filter->Filter as $filterItem) {
											$tempFilterStr = $filter->GetFilterQueryString($filterItem->ID);

											echo sprintf('<div class="FilterOption"><a href="%s%s"><div class="FilterOptionCross FilterOptionCross4">%s: %s</div></a></div>', $searchPage, (strlen($tempFilterStr) > 0) ? sprintf('?%s', $tempFilterStr) : '', $filterItem->Group->Name, $filterItem->GetUnitValue());
										}
									...
									if(count($filter->Category) > 0) {
										...
										foreach($filter->Category as $categoryItem) {
											...
											echo sprintf('<div class="FilterOption"><a href="%s%s"><div class="FilterOptionCross FilterOptionCross3">%s</div></a></div>', $searchPage, (strlen($tempFilterStr) > 0) ? sprintf('?%s', $tempFilterStr) : '', $categoryItem->Name);
										}
											...
											for($j=0; $j<count($columnArr[0]); $j++) {
												...
												for($k=0; $k<$maxColumns; $k++) {
													if(isset($columnArr[$k][$j])) {
														echo sprintf('<td width="%s%%" valign="top"><a href="%s?%sfilter=%d">%s</a> (%d)</td>', number_format(100/$maxColumns, 2, '.', ''), $searchPage, (strlen($filter->FilterQueryString) > 0) ? sprintf('%s&', $filter->FilterQueryString) : '', $columnArr[$k][$j]['Value_ID'], $columnArr[$k][$j]['UnitValue'], $columnArr[$k][$j]['Products']);
													} else {
														echo '<td>&nbsp;</td>';
													}
												}
