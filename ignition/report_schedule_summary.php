...
function start(){
	...
	$page = new Page('Schedule Summary Report', 'Please choose a start and end date for your report');
...
function report($start, $end){
	...
	$page = new Page('Schedule Summary Report: ' . cDatetime($start, 'longdatetime') . ' to ' . cDatetime($end, 'longdatetime'), '');
	$page->Display('header');
	...
	foreach($accountManagers as $accountManagerId=>$accountManagerName) {
		...
		new DataQuery(sprintf("CREATE TEMPORARY TABLE temp_schedule SELECT DATE(cs.Completed_On) AS Completed_On, cs.Contact_Schedule_Type_ID FROM contact_schedule AS cs WHERE cs.Owned_By=%d AND cs.Is_Complete='Y' AND cs.Completed_On BETWEEN '%s' AND '%s'", mysql_real_escape_string($accountManagerId), mysql_real_escape_string($start), mysql_real_escape_string($end)));
		...
		if(count($schedules) > 0) {
			...
			<table width="100%" border="0" >
				<tr>
					<td style="border-bottom:1px solid #aaaaaa"><strong>Completed</strong></td>

					<?php
					foreach($scheduleTypes as $scheduleTypeId=>$scheduleType) {
						echo sprintf('<td style="border-bottom:1px solid #aaaaaa" align="center"><strong>%s</strong></td>', $scheduleType);
					}
					?>
				...
				foreach($schedules as $completedDate=>$scheduleItem) {
					...
					<tr class="dataRow" onMouseOver="setClassName(this, 'dataRowOver');" onMouseOut="setClassName(this, 'dataRow');">
						...
						<?php
						foreach($scheduleTypes as $scheduleTypeId=>$scheduleType) {
							echo sprintf('<td align="center">%s</td>', $scheduleItem[$scheduleTypeId]);
						}
						?>
				...
				<tr class="dataRow" onMouseOver="setClassName(this, 'dataRowOver');" onMouseOut="setClassName(this, 'dataRow');">
					...
					<?php
					foreach($scheduleTypes as $scheduleTypeId=>$scheduleType) {
						echo sprintf('<td align="center"><strong>%s</strong></td>', $totals[$scheduleTypeId]);
					}
					?>
