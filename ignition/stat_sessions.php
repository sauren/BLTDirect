...
$sqlFrom = sprintf("FROM customer_session_archive AS cs LEFT JOIN customer_session_item_archive AS csi ON csi.Session_ID=cs.Session_ID ");
...
if($form->GetValue('customer') > 0) {
	$sqlFrom .= sprintf("INNER JOIN customer_session_item_archive AS csi2 ON csi2.Session_ID=csi.Session_ID AND csi2.Customer_ID=%d ", mysql_real_escape_string($form->GetValue('customer')));
}
