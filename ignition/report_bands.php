...
function start(){
	...
	$page = new Page('Product Bands Report', 'Please choose a start and end date for your report');
...
function report($start, $end){
	...
	$page = new Page('Product Bands Report : ' . cDatetime($start, 'longdatetime') . ' to ' . cDatetime($end, 'longdatetime'), '');
	$page->AddToHead('<script language="javascript" src="js/scw.js" type="text/javascript"></script>');
$page->Display('header');

	$data = new DataQuery(sprintf("SELECT pb.Band_Ref, COUNT(*) AS Orders, SUM(Line_Total-Line_Discount) AS Total_Order, SUM(sp.Cost*ol.Quantity) AS Total_Cost FROM orders AS o INNER JOIN order_line AS ol ON ol.Order_ID=o.Order_ID INNER JOIN product AS p ON ol.Product_ID=p.Product_ID INNER JOIN product_band AS pb ON p.Product_Band_ID=pb.Product_Band_ID INNER JOIN supplier_product AS sp ON sp.Product_ID=p.Product_ID WHERE o.Ordered_On BETWEEN '%s' AND '%s' AND (ol.Line_Status<>'Cancelled' OR ol.Line_Status<>'Backordered') AND o.Status<>'Cancelled' AND sp.Preferred_Supplier='Y' GROUP BY pb.Product_Band_ID ORDER BY Band_Description ASC", $start, $end));
	?>

	<br />
	<h3>Product Band Totals</h3>
	<p>Listing product band details for the selected period.</p>
	<table width="100%" border="0" >
	  <tr>
		<td style="border-bottom:1px solid #aaaaaa" align="left" valign="top"><strong>Band</strong><br />Reference</td>
		<td style="border-bottom:1px solid #aaaaaa" align="left" valign="top"><strong>Orders</strong><br />Frequency of orders containing distinct banded products</td>
		<td style="border-bottom:1px solid #aaaaaa" align="left" valign="top"><strong>Total Cost</strong><br />Based on current cost</td>
		<td style="border-bottom:1px solid #aaaaaa" align="left" valign="top"><strong>Total Sale</strong><br />Sale price of products</td>
		<td style="border-bottom:1px solid #aaaaaa" align="left" valign="top"><strong>Total Profit</strong><br />Based on current cost</td>
		<td style="border-bottom:1px solid #aaaaaa" align="left" valign="top"><strong>Average Markup</strong><br />((<em>price</em>/<em>cost</em>)*100)-100</td>
	  </tr>
		...
		while($data->Row) {
			?>
			 ...
			 <tr class="dataRow" onMouseOver="setClassName(this, 'dataRowOver');" onMouseOut="setClassName(this, 'dataRow');">
				<td align="left"><?php print $data->Row['Band_Ref']; ?></td>
				<td align="right"><?php print number_format($data->Row['Orders'], 0, '.', ','); ?></td>
				<td align="right">&pound;<?php print number_format($data->Row['Total_Cost'], 2, '.', ','); ?></td>
				<td align="right">&pound;<?php print number_format($data->Row['Total_Order'], 2, '.', ','); ?></td>
				<td align="right">&pound;<?php print number_format(($data->Row['Total_Order']-$data->Row['Total_Cost']), 2, '.', ','); ?></td>
				<td align="right"><?php print number_format(((($data->Row['Total_Order']/$data->Row['Total_Cost'])*100)-100), 2, '.', ','); ?>%</td>
			  </tr>
