...
function start(){
	...
	if(isset($_REQUEST['confirm'])) {
		if(($form->GetValue('range') != 'none') && (strlen($form->GetValue('range')) > 1)) {
			...
			redirect(sprintf('Location: ?action=report&start=%s&end=%s&products=%d', $start, $end, $form->GetValue('products')));
		} else {
			if($form->Validate()) {
				redirect(sprintf('Location: ?action=report&start=%s&end=%s&products=%d', sprintf('%s-%s-%s 00:00:00', substr($form->GetValue('start'), 6, 4), substr($form->GetValue('start'), 3, 2), substr($form->GetValue('start'), 0, 2)), (strlen($form->GetValue('end')) > 0) ? sprintf('%s-%s-%s 00:00:00', substr($form->GetValue('end'), 6, 4), substr($form->GetValue('end'), 3, 2), substr($form->GetValue('end'), 0, 2)) : date('Y-m-d H:i:s', mktime(0, 0, 0, date('m', strtotime(sprintf('%s-%s-%s 00:00:00', substr($form->GetValue('start'), 6, 4), substr($form->GetValue('start'), 3, 2), substr($form->GetValue('start'), 0, 2)))), date('d', strtotime(sprintf('%s-%s-%s 00:00:00', substr($form->GetValue('start'), 6, 4), substr($form->GetValue('start'), 3, 2), substr($form->GetValue('start'), 0, 2)))) + 1, date('Y', strtotime(sprintf('%s-%s-%s 00:00:00', substr($form->GetValue('start'), 6, 4), substr($form->GetValue('start'), 3, 2), substr($form->GetValue('start'), 0, 2)))))), $form->GetValue('products'), $form->GetValue('products')));
			}
		}
	}

	$page = new Page('Lone Products Report', 'Please choose a start and end date for your report');
	...
	$window = new StandardWindow("Report on Lone Products.");
...
function report() {
	...
	$form->AddField('products', 'Products', 'hidden', '0', 'numeric_unsigned', 1, 11);

	$page = new Page('Lone Products Report: ' . cDatetime($form->GetValue('start'), 'longdatetime') . ' to ' . cDatetime($form->GetValue('end'), 'longdatetime'), '');
	$page->Display('header');
	?>
	
	<br />
	<h3>Lone Products</h3>
	<p>Listing product sold alone in orders between the specified dates.</p>
	...
	<table width="100%" cellspacing="0">
		...
		<?php
		$data = new DataQuery(sprintf("SELECT q.Product_ID, p.Product_Title, SUM(q.Quantity) AS Quantity, COUNT(q.Order_ID) AS Orders FROM (SELECT ol.Product_ID, ol.Quantity, o.Order_ID FROM orders AS o INNER JOIN order_line AS ol ON ol.Order_ID=o.Order_ID WHERE ol.Product_ID>0 AND o.Status NOT IN ('Cancelled', 'Incomplete', 'Unauthenticated') AND o.Created_On>='%s' AND o.Created_On<'%s' GROUP BY o.Order_ID HAVING COUNT(DISTINCT ol.Product_ID)=1) AS q LEFT JOIN product AS p ON p.Product_ID=q.Product_ID GROUP BY q.Product_ID ORDER BY Orders DESC LIMIT 0, %d", $form->GetValue('start'), $form->GetValue('end'), mysql_real_escape_string($form->GetValue('products'))));
		if($data->TotalRows > 0) {
			$totalOrders = 0;
			...
			while($data->Row) {
				...
				<tr>
					<td style="border-bottom: 1px dashed #aaaaaa; padding: 5px;"><a href="product_profile.php?pid=<?php echo $data->Row['Product_ID']; ?>" target="_blank"><?php echo strip_tags($data->Row['Product_Title']); ?></a></td>
					<td style="border-bottom: 1px dashed #aaaaaa; padding: 5px;" align="right"><?php echo $data->Row['Product_ID']; ?></td>
					<td style="border-bottom: 1px dashed #aaaaaa; padding: 5px;" align="right"><?php echo $data->Row['Orders']; ?></td>
					<td style="border-bottom: 1px dashed #aaaaaa; padding: 5px;" align="right"><?php echo $data->Row['Quantity']; ?></td>
				</tr>
			...
			<tr>
				<td style="border-bottom: 1px dashed #aaaaaa; padding: 5px;"></td>
				<td style="border-bottom: 1px dashed #aaaaaa; padding: 5px;"></td>
				<td style="border-bottom: 1px dashed #aaaaaa; padding: 5px;" align="right"><strong><?php echo $totalOrders; ?></strong></td>
				<td style="border-bottom: 1px dashed #aaaaaa; padding: 5px;"></td>
			</tr>
