...
function start(){
	...
	$page = new Page('PDQ Payments Report', 'Please choose a start and end date for your report');
	...
	$window = new StandardWindow("Report on PDQ Payments.");
...
function report($start, $end){
	...
	$page = new Page('PDQ Payments Report : ' . cDatetime($start, 'longdatetime') . ' to ' . cDatetime($end, 'longdatetime'), 'This report shows payments taken manually via PDQ machine.');
	$page->Display('header');
	
	$paymentMethod = new PaymentMethod;
	$paymentMethod->GetByReference('pdq');
	$sql = sprintf(Order::getSql("i.Invoice_Total, i.Invoice_ID") . " join invoice as i on i.Order_ID=o.Order_ID WHERE i.Created_On between '%s' and '%s' and o.Payment_Method_ID={$paymentMethod->ID} order by o.Order_ID", mysql_real_escape_string($start), mysql_real_escape_string($end));
	
	$data = new DataQuery($sql);
	
	$receiptSent = array();
	$receiptRequired = array();
	
	while($data->Row){
		$order = new Order;
		$order->populate($data->Row);
		// add additional stuff in
		$order->InvoiceID = $data->Row['Invoice_ID'];
		if(md5(serialize($order->Invoice->Address)) == md5(serialize($order->Shipping->Address))){
			$receiptSent[] = $order;
		} else {
			$receiptRequired[] = $order;
		}
		$data->Next();
	}
	$data->Disconnect();
	...
	<p>The customer receipt <u>needs</u> to be sent to the customer.</p>
	...
	<table width="100%" border="0" cellpadding="3" cellspacing="0">
	  <tr style="background-color:#eeeeee;">
		...
		<td style="border-bottom:1px solid #dddddd;"><strong>Ordered</strong></td>
		<td style="border-bottom:1px solid #dddddd;"><strong>Customer</strong></td>
		<td align="right" style="border-bottom:1px solid #dddddd;"><strong>Amount</strong></td>
	  </tr>
	  <?php foreach($receiptRequired as $order){ ?>
	  <tr>
	  	<td style="border-bottom:1px solid #dddddd;"><a href="order_details.php?orderid=<?php echo $order->ID; ?>"><?php echo $order->ID; ?></a></td>
		<td style="border-bottom:1px solid #dddddd;"><a href="invoice.php?invoiceid=<?php echo $order->InvoiceID; ?>"><?php echo $order->InvoiceID; ?></a></td>
		<td style="border-bottom:1px solid #dddddd;"><?php echo fromNow($order->CreatedOn); ?></td>
		<td style="border-bottom:1px solid #dddddd;"><?php echo $order->Billing->GetFullName(); ?></td>
		<td align="right" style="border-bottom:1px solid #dddddd;"><?php echo $order->Total; ?></td>
	  </tr>
	  <?php } ?>
	</table>
	
	<br />
	<h3>Receipt Sent with Shipment</h3>
	<p>The customer receipt <u>should</u> have been sent with the shipment for the following orders.</p>
	...
	<table width="100%" border="0" cellpadding="3" cellspacing="0">
	  ...
	  <?php foreach($receiptSent as $order){ ?>
	  <tr>
	  	<td style="border-bottom:1px solid #dddddd;"><a href="order_details.php?orderid=<?php echo $order->ID; ?>"><?php echo $order->ID; ?></a></td>
		<td style="border-bottom:1px solid #dddddd;"><a href="invoice.php?invoiceid=<?php echo $order->InvoiceID; ?>"><?php echo $order->InvoiceID; ?></a></td>
		<td style="border-bottom:1px solid #dddddd;"><?php echo fromNow($order->CreatedOn); ?></td>
