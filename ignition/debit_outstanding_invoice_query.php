...
$page = new Page('Pending Debits (Invoice Queries)', 'Below is a list of all invoice query debits missing transaction references.');
$page->Display('header');
...
$table->SetSQL("SELECT d.*, CONCAT(d.Prefix, d.Debit_ID) AS Debit_Reference FROM debit AS d LEFT JOIN supplier_invoice_query AS siq ON siq.DebitID=d.Debit_ID WHERE d.Is_Paid='Y' AND d.Integration_ID='' AND siq.SupplierInvoiceQueryID IS NOT NULL GROUP BY d.Debit_ID");
