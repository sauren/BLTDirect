...
if(isset($_REQUEST['confirm']) && (strtolower($_REQUEST['confirm']) == 'true')) {
	if($form->Validate()) {
		...
		$sqlFrom = sprintf("FROM despatch AS d INNER JOIN orders AS o ON o.Order_ID=d.Order_ID INNER JOIN warehouse AS w ON w.Warehouse_ID=d.Despatch_From_ID AND w.Type='S' INNER JOIN courier AS c ON c.Courier_ID=d.Courier_ID ");
		$sqlWhere = sprintf("WHERE w.Type_Reference_ID=%d ", $session->Supplier->ID);
		...
		if($data->TotalRows > 0) {
			...
			if(isset($_REQUEST[$key])) {
				...
				if(strtolower(trim($data->Row['Consignment'])) != strtolower(trim($value))) {
					...
					new DataQuery(sprintf("UPDATE despatch SET Consignment='%s' WHERE Despatch_ID=%d", mysql_real_escape_string($value), $despatchId));
				}
			}
...
$page = new Page('Track Consignments', 'Track despatch consignment details here.');
$page->Display('header');
