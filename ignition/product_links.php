...
function remove(){
	if(isset($_REQUEST['id'])) {
		$link = new ProductLink($_REQUEST['id']);
		$link->delete();
		
		redirect(sprintf("Location: ?pid=%d", $link->product->ID));
	}
	
	redirect('Location: product_search.php');
}
...
function add() {
	...
	if(isset($_REQUEST['confirm'])) {
		if($form->Validate()) {
			...
			$link->product->ID = $form->GetValue('pid');
	...
	$page = new Page(sprintf('<a href="product_profile.php?pid=%s">Product Profile</a> &gt; <a href="?pid=%s">Product Links</a> &gt; Add Link', $form->GetValue('pid'), $form->GetValue('pid')), 'Add new product link.');
	$page->Display('header');
	...
	echo $webForm->AddRow('', sprintf('<input type="button" name="back" value="back" class="btn" onclick="window.self.location = \'?pid=%s\';" /> <input type="submit" name="add" value="add" class="btn" tabindex="%s" />', $form->GetValue('pid'), $form->GetTabIndex()));
	echo $webForm->Close();
...
function update() {
	...
	$page = new Page(sprintf('<a href="product_profile.php?pid=%s">Product Profile</a> &gt; <a href="?pid=%s">Product Links</a> &gt; Update Link', $link->product->ID, $link->product->ID), 'Update existing product link.');
	$page->Display('header');
	...
	echo $webForm->AddRow($form->GetLabel('image'), $form->GetHTML('image') . $form->GetIcon('image'));
	echo $webForm->AddRow('', sprintf('<input type="button" name="back" value="back" class="btn" onclick="window.self.location = \'?pid=%s\';" /> <input type="submit" name="update" value="update" class="btn" tabindex="%s" />', $link->product->ID, $form->GetTabIndex()));
	echo $webForm->Close();
	echo $window->CloseContent();
	echo $window->Close();
...
function view() {
	...
	$page = new Page(sprintf('<a href="product_profile.php?pid=%s">Product Profile</a> &gt; Product Links', $_REQUEST['pid']), 'Manage product links here.');
	$page->Display('header');

	$table = new DataTable('links');
	$table->SetSQL(sprintf("SELECT * FROM product_link WHERE productId=%d", $_REQUEST['pid']));
	...
	echo '<br />';
	echo sprintf('<input type="button" name="add" value="add new link" class="btn" onclick="window.location.href=\'?action=add&pid=%d\'" />', $_REQUEST['pid']);
