    ...
    */
    ...
    class PorterStemmer
    {
        ...
        */
		...
		public static function StemStages($word){
			if (strlen($word) <= 2) {
                return $word;
            }
			
			$words = array($word);
            $word1 = self::step1ab($word);
			if(!in_array($word1, $words)) $words[] = $word1;
            $word2 = self::step1c($word1);
			if(!in_array($word2, $words)) $words[] = $word2;
            $word3 = self::step2($word2);
			if(!in_array($word3, $words)) $words[] = $word3;
            $word4 = self::step3($word3);
			if(!in_array($word4, $words)) $words[] = $word4;
            $word5 = self::step4($word4);
			if(!in_array($word5, $words)) $words[] = $word5;
            $word6 = self::step5($word5);
			if(!in_array($word6, $words)) $words[] = $word6;
			
			// customised
			$word7 = self::stepCustom1($word);
			if(!in_array($word7, $words)) $words[] = $word7;
			
			return $words;
		}
		
		private static function stepCustom1($word){
			if (substr($word, -1) == 's') {
				self::replace($word, 'ies', 'y');
			}
			return $word;
		}
        ...
        private static function step2($word)
        {
            switch (substr($word, -2, 1)) {
                ...
                case 'e':
					...
					OR self::replace($word, 'iser', 'ise', 0);
                    break;
                ...
                case 'o':
                    ...
                    OR self::replace($word, 'isation', 'ise', 0)
        ...
        private static function step3($word)
        {
            switch (substr($word, -2, 1)) {
                ...
                case 's':
					...
					OR self::replace($word, 'alise', 'al', 0);
                    break;
