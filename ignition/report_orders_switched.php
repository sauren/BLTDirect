...
function start(){
	...
	$page = new Page('Orders Switched Report', 'Please choose a start and end date for your report');
...
function report($start, $end) {
	...
	$page = new Page('Orders Switched Report: ' . cDatetime($start, 'longdatetime') . ' to ' . cDatetime($end, 'longdatetime'), '');
	$page->Display('header');
	?>
	...
	<table width="100%" border="0">
		<tr>
			...
			<td style="border-bottom: 1px solid #aaaaaa;"><strong>Original Product</strong></td>
			<td style="border-bottom: 1px solid #aaaaaa;"><strong>Original Quickfind</strong></td>
		...
		<?php
		$data = new DataQuery(sprintf("SELECT o.Order_ID, o.Order_Prefix, ol.Product_ID, ol.Product_Title, ol.Quantity, ol.Original_Product_ID, p.Product_Title AS Original_Product_Title FROM orders AS o INNER JOIN order_line AS ol ON ol.Order_ID=o.Order_ID INNER JOIN product AS p ON p.Product_ID=ol.Original_Product_ID WHERE o.Created_On>='%s' AND o.Created_On<'%s'", $start, $end));
		while($data->Row) {
			?>

			<tr class="dataRow">
				<td><a href="order_details.php?orderid=<?php echo $data->Row['Order_ID']; ?>"><?php echo $data->Row['Order_Prefix'].$data->Row['Order_ID']; ?></a></td>
				<td><?php echo $data->Row['Original_Product_Title']; ?></td>
				<td><a href="product_profile.php?pid=<?php echo $data->Row['Original_Product_ID']; ?>"><?php echo $data->Row['Original_Product_ID']; ?></td>
				<td><?php echo $data->Row['Quantity']; ?></td>
				<td><?php echo strip_tags($data->Row['Product_Title']); ?></td>
				<td><a href="product_profile.php?pid=<?php echo $data->Row['Product_ID']; ?>"><?php echo $data->Row['Product_ID']; ?></td>
			</tr>
