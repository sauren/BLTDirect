...
function start(){
	...
	$page = new Page('Supplier Costs Report', 'Please choose a start and end date for your report');
	...
	$window = new StandardWindow("Report on Supplier Costs.");
...
function report($start, $end) {
	...
	$page = new Page('Supplier Costs Report: ' . cDatetime($start, 'longdatetime') . ' to ' . cDatetime($end, 'longdatetime'), '');
	$page->Display('header');
	?>

	<br />
	<h3>Supplier Costs Summary</h3>
	<p>Totalling supplier costs for the given period.</p>
	...
	<table width="100%" border="0">
		...
		<?php
		$data = new DataQuery(sprintf("SELECT s.Supplier_ID, CONCAT_WS(' ', p.Name_First, p.Name_Last, CONCAT('(', o.Org_Name, ')')) AS Supplier, SUM(ol.Quantity * ol.Cost) AS Cost FROM supplier AS s INNER JOIN warehouse AS w ON s.Supplier_ID=w.Type_Reference_ID AND w.Type='S' INNER JOIN order_line AS ol ON ol.Despatch_From_ID=w.Warehouse_ID INNER JOIN despatch AS d ON d.Despatch_ID=ol.Despatch_ID AND d.Created_On>='%s' AND d.Created_On<'%s' INNER JOIN contact AS c ON c.Contact_ID=s.Contact_ID INNER JOIN contact AS c2 ON c.Parent_Contact_ID=c2.Contact_ID INNER JOIN person AS p ON p.Person_ID=c.Person_ID INNER JOIN organisation AS o ON o.Org_ID=c2.Org_ID GROUP BY s.Supplier_ID", mysql_real_escape_string($start), mysql_real_escape_string($end)));
		while($data->Row) {
			?>
