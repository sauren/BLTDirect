...
<table width="100%" border="0">
  <tr>
    ...
    <?php if(param('type')=='billing'){ ?>
    	<strong>Billing Address</strong>
			<p>If the detail below are not those of customer's please <a href="order_register.php">click here to make a new registration</a>. Please select your billing address from below. You may also add additional billing addresses, which will be kept for use later.</p>
    ...
    <?php } else {?>
			...
			<?php
				if(param('status')=='update' && param('type')=='p'){?>
				<div class="detailNotification"> 
			        <h1>Customer Details Missing</h1>
			        <p> You have been redirected back to this page as the details that have provided are incomplete or are not valid. Please ammend and save the changes to the correct format/required  fields before proceeding with the order.</p>
			    </div>
			    <br/>
			<?php } else if(param('status')=='update' && param('type')=='s'){?>
				<div class="detailNotification"> 
			        <h1>Shipping Address Missing</h1>
			        <p> You have been redirected back to this page as the shipping address seleted is incomplete or not valid. Please ammend and save the changes to the correct format/required  fields before proceeding with the order.</p>
			    </div>
			    <br/>
			<?php }
		}?>
			...
			<?php
			$contacts = new DataQuery(sprintf("select * from customer_contact where Customer_ID=%d", mysql_real_escape_string($cart->Customer->ID)));
			while ($contacts->Row){
