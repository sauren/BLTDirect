...
function view() {
	$page = new Page('Unauthenticated Orders', 'Listing all unauthenticated orders.');
	$page->Display('header');
	
	$table = new DataTable('orders');
	$table->SetSQL("SELECT o.*, pg.Postage_Title, pg.Postage_Days, DATE(MAX(oc.CreatedOn)) AS Contacted_On FROM orders AS o LEFT JOIN postage AS pg ON o.Postage_ID=pg.Postage_ID LEFT JOIN order_contact AS oc ON oc.OrderID=o.Order_ID WHERE o.Status IN ('Incomplete', 'Unauthenticated') AND o.Is_Dismissed = 'N' GROUP BY o.Order_ID");
