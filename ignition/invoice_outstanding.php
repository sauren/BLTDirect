...
function view() {
	...
	$invoiceSql = sprintf("SELECT *, CONCAT_WS(' ', Invoice_First_Name, Invoice_Last_Name) AS Invoice_Name FROM invoice WHERE Integration_ID='' AND Is_Paid='N'");
	...
	$page = new Page('Outstanding Invoices', 'Below is a list of all invoices currently outstanding.');
	$page->Display('header');
...
function printAll() {
	...
	$data = new DataQuery(sprintf("SELECT Invoice_ID FROM invoice WHERE Integration_ID='' AND Is_Paid='N'"));
	if($data->TotalRows > 0) {
