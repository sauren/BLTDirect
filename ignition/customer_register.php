...
$form->AddField('businesscountry', 'Country', 'select', $address->Country->ID, 'numeric_unsigned', 1, 11, true, 'onChange="propogateRegions(\'businessregion\', this);"');
...
if($regionCount > 0){
	$form->AddField('businessregion', 'Region', 'select', $address->Region->ID, 'numeric_unsigned', 1, 11, true);
	...
	$form->AddField('businessregion', 'Region', 'select', '', 'numeric_unsigned', 1, 11, true, 'disabled="disabled"');
	$form->AddOption('businessregion', '0', '');
}
$region->Disconnect();
...
$regionCount = $region->TotalRows;
if($regionCount > 0){
	$form->AddField('accountsregion', 'Region', 'select', '', 'numeric_unsigned', 1, 11, ($isAsAccount)?true:false, ($form->GetValue('accountsinvoice') == 'N') ? 'disabled="disabled"' : '');
...
$form->AddField('country', 'Country', 'select', $address->Country->ID, 'numeric_unsigned', 1, 11, true, $str);
...
$regionCount = $region->TotalRows;
if($regionCount > 0){
	$form->AddField('region', 'Region', 'select', $address->Region->ID, 'numeric_unsigned', 1, 11, true);
	...
	$form->AddField('region', 'Region', 'select', '', 'numeric_unsigned', 1, 11, true, 'disabled="disabled"');
	$form->AddOption('region', '0', '');
}
$region->Disconnect();

$form->AddField('istest', 'Is Test', 'checkbox', 'N', 'boolean', 1, 1, false);
...
if(isset($_REQUEST['confirm']) && strtolower($_REQUEST['confirm']) == "true"){
	...
	$customer->Contact->IsTest = $form->GetValue('istest');
	...
	if($form->Valid){
		...
		if($isOrg){
			...
			$contact->Organisation->Division = $form->GetValue('division');
			$contact->Add();
		...
		redirect(sprintf("Location: customer_registered.php?customerid=%d", $customer->ID));
	}
}
...
$page = new Page('Create New Customer', '');
...
?>
	...
	<strong>Create New Customer Account</strong>
			<p>To create a new customer account please complete the following form.</p>
			...
			<table width="100%" cellspacing="0" class="form">
              ...
              <tr>
							...
							<tr id="position" style="display: <?php echo ($isBusiness) ? 'table-row' : 'none'; ?>;">
		...
		<table width="100%" cellspacing="0" class="form">
					...
					<tr>
						<td><?php echo $form->GetLabel('division'); ?></td>
						<td><?php echo $form->GetHtml('division'); ?> <?php echo $form->GetIcon('division'); ?></td>
						  </tr>
					<tr>
			  ...
			  <table width="100%" cellspacing="0" class="form">
                        ...
                        <th colspan="2">Accounts Details </th>
							...
							<tr>
								...
								<a href="javascript:Address.find(document.getElementById('accountspostcode'));"><img src="../images/searchIcon.gif" border="0" align="absmiddle" />Auto-complete address (UK residents)</a> </td>
							</tr>
							<tr>
					...
					<table width="100%" cellspacing="0" class="form">
                      ...
                      <tr id="asbusinessRow" style="display:<?php echo ($isBusiness)?'block':'none';?>">
                        ...
                        <td width="72%"><?php echo $form->GetHtml('asBusiness'); ?> <?php echo $form->GetIcon('asBusiness'); ?></td>
                      </tr>
                      <tr>
                    ...
                    <table width="100%" cellspacing="0" class="form">
                      ...
                      <tr>
                        <td width="28%"><?php echo $form->GetLabel('istest'); ?> </td>
                        <td width="72%"><?php echo $form->GetHtml('istest'); ?> <?php echo $form->GetIcon('istest'); ?></td>
                      </tr>
