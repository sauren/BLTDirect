...
function start(){
	...
	$page = new Page('Warehouse Despatch Report', 'Please choose a start and end date for your report');
	...
	$window = new StandardWindow("Report on Warehouse Despatches.");
...
function report($start, $end){
	...
	$data = new DataQuery(sprintf("SELECT Warehouse_ID, Warehouse_Name FROM warehouse ORDER BY Warehouse_Name ASC", mysql_real_escape_string($start), mysql_real_escape_string($end)));
	while($data->Row) {
		$warehouse[] = $data->Row;
	...
	$page = new Page('Warehouse Despatch Report: ' . cDatetime($start, 'longdatetime') . ' to ' . cDatetime($end, 'longdatetime'), '');
	$page->Display('header');

	foreach($warehouse as $warehouseItem) {
		$data = new DataQuery(sprintf("SELECT ol.Product_ID, ol.Product_Title, ol.Cost, SUM(ol.Quantity) AS Quantity FROM orders AS o INNER JOIN order_line AS ol ON o.Order_ID=ol.Order_ID AND ol.Despatch_From_ID=%d WHERE o.Created_On>='%s' AND o.Created_On<'%s' GROUP BY ol.Product_ID, ol.Cost", $warehouseItem['Warehouse_ID'], mysql_real_escape_string($start), mysql_real_escape_string($end)));
		if($data->TotalRows > 0) {
			?>

			<br />
			<h3><?php echo $warehouseItem['Warehouse_Name']; ?></h3>
			<p>Listing all product costing details despatched by the chosen warehouse between the selected time frame.</p>

			<table width="100%" border="0" >
				<tr>
					<td style="border-bottom:1px solid #aaaaaa;" width="10%"><strong>Product ID</strong></td>
					<td style="border-bottom:1px solid #aaaaaa;"><strong>Name</strong></td>
					<td style="border-bottom:1px solid #aaaaaa;" width="10%"><strong>Quantity</strong></td>
					<td style="border-bottom:1px solid #aaaaaa; text-align: right;" width="10%"><strong>Cost</strong></td>
					<td style="border-bottom:1px solid #aaaaaa; text-align: right;" width="10%"><strong>Line Cost</strong></td>
				</tr>
