...
function start(){
	...
	$page = new Page('Order Markup Report', 'Please choose a start and end date for your report');
...
function report() {
	...
	$page = new Page('Order Markup Report: ' . cDatetime($form->GetValue('start'), 'longdatetime') . ' to ' . cDatetime($form->GetValue('end'), 'longdatetime'), '');
	$page->Display('header');
	?>

	<br />
	<h3>Markup On Products</h3>
	<p>Listing markup on discounted products ordered by the selected user for the chosen period.</p>
	...
	<table width="100%" border="0">
		<tr>
			...
			<td style="border-bottom: 1px solid #aaaaaa;" align="right"><strong>Qty</strong></td>
			...
			<td style="border-bottom: 1px solid #aaaaaa;" align="right"><strong>Markup</strong><br />Original</td>
			<td style="border-bottom: 1px solid #aaaaaa;" align="right"><strong>Markup</strong><br />Post Discount</td>
			<td style="border-bottom: 1px solid #aaaaaa;" align="right"><strong>Markup</strong><br />Post Discount Best Cost</td>
		</tr>

		<?php
		$data = new DataQuery(sprintf("SELECT o.Order_Prefix, o.Order_ID, o.SubTotal, CONCAT_WS(' ', o.Billing_First_Name, o.Billing_Last_Name) AS Customer, DATE(o.Created_On) AS Created_Date, p.Product_ID, p.Product_Title, ol.Quantity, ol.Line_Discount, ol.Line_Total, ol.Line_Total-ol.Line_Discount AS Total, ol.Cost*ol.Quantity AS Cost, ol.Cost_Best*ol.Quantity AS CostBest FROM orders AS o INNER JOIN order_line AS ol ON ol.Order_ID=o.Order_ID INNER JOIN product AS p ON p.Product_ID=ol.Product_ID WHERE o.Created_By=%d AND o.Created_On>'%s' AND o.Created_On<'%s' AND o.Status NOT IN ('Cancelled', 'Incomplete', 'Unauthenticated' AND o.Order_Prefix<>'R' AND o.Order_Prefix<>'B' AND o.Order_Prefix<>'N' ORDER BY o.Order_ID ASC", mysql_real_escape_string($form->GetValue('user')), mysql_real_escape_string($form->GetValue('start')), mysql_real_escape_string($form->GetValue('end'))));
		if($data->TotalRows > 0) {
			while($data->Row) {
				$profit = (($data->Row['Total'] - $data->Row['Cost']) / $data->Row['Total']) * 100;
				$markupOriginal = ($data->Row['Cost'] > 0) ? (($data->Row['Line_Total'] - $data->Row['Cost']) / $data->Row['Cost']) * 100 : 0;
				$markupPost = ($data->Row['Cost'] > 0) ? (($data->Row['Total'] - $data->Row['Cost']) / $data->Row['Cost']) * 100 : 0;
				$markupPostBest = ($data->Row['CostBest'] > 0) ? (($data->Row['Total'] - $data->Row['CostBest']) / $data->Row['CostBest']) * 100 : 0;
				...
				if($markupPost < 30) {
					$colourBackground = 'ff9';
					$colourText = ($data->Row['Line_Discount'] > 0) ? 'f00' : '00f';
					$fontWeight = 'bold';
				}
				?>

				<tr style="background-color: <?php echo $colourBackground; ?>;">
					...
					<td align="right"><?php echo number_format(round($profit, 2), 2, '.', ','); ?>%</td>
					<td align="right"><?php echo number_format(round($markupOriginal, 2), 2, '.', ','); ?>%</td>
					<td align="right" style="color: <?php echo $colourText; ?>; font-weight: <?php echo $fontWeight; ?>"><?php echo number_format(round($markupPost, 2), 2, '.', ','); ?>%</td>
					<td align="right"><?php echo number_format(round($markupPostBest, 2), 2, '.', ','); ?>%</td>
				</tr>
