/*
	...
	Notes:
	The following functions do not integrate with the CoolJsTreePro
	they are used by Ignition in part or in full.
*/

function treeMenuSettings(){
	this.currentNodeId = 0;
	this.currentNodeStr = "_root";
	...
	this.drawOptions = function(){
		if(this.currentNodeId == 0){
			document.getElementById("optionRemove").style.display = "none";
			document.getElementById("optionUpdate").style.display = "none";

			if(document.getElementById("optionMoveUp")) {
				document.getElementById("optionMoveUp").style.display = "none";
			}

			if(document.getElementById("optionMoveDown")) {
				document.getElementById("optionMoveDown").style.display = "none";
			}

			if(document.getElementById("optionDuplicate")) {
				document.getElementById("optionDuplicate").style.display = "none";
			}
			
			if(document.getElementById("optionSpec")) {
				document.getElementById("optionSpec").style.display = "none";
			}
			
			if(document.getElementById("optionLink")) {
				document.getElementById("optionLink").style.display = "none";
			}
			
			if(document.getElementById("optionCatalogueImages")) {
				document.getElementById("optionCatalogueImages").style.display = "none";
			}

			if(document.getElementById("optionManageRelated")) {
				document.getElementById("optionManageRelated").style.display = "none";
			}

			if(document.getElementById("optionManageLinks")) {
				document.getElementById("optionManageLinks").style.display = "none";
			}

			if(document.getElementById("optionQuality")) {
				document.getElementById("optionQuality").style.display = "none";
			}
		} else {
			document.getElementById("optionRemove").style.display = "";
			document.getElementById("optionUpdate").style.display = "";

			if(document.getElementById("optionMoveUp")) {
				document.getElementById("optionMoveUp").style.display = "";
			}

			if(document.getElementById("optionMoveDown")) {
				document.getElementById("optionMoveDown").style.display = "";
			}

			if(document.getElementById("optionDuplicate")) {
				document.getElementById("optionDuplicate").style.display = "";
			}
			
			if(document.getElementById("optionSpec")) {
				document.getElementById("optionSpec").style.display = "";
			}
			
			if(document.getElementById("optionLink")) {
				document.getElementById("optionLink").style.display = "";
			}
			
			if(document.getElementById("optionCatalogueImages")) {
				document.getElementById("optionCatalogueImages").style.display = "";
			}

			if(document.getElementById("optionManageRelated")) {
				document.getElementById("optionManageRelated").style.display = "";
			}

			if(document.getElementById("optionManageLinks")) {
				document.getElementById("optionManageLinks").style.display = "";
			}

			if(document.getElementById("optionQuality")) {
				document.getElementById("optionQuality").style.display = "";
			}
		}
	}

	this.selectOption = function(url){
		document.location.href = url + this.currentNodeId;
	}

	this.remove = function(url){
		var txt = "You have requested permanent removal of " + this.currentNodeStr + " and its child nodes. If you only want to disable " + this.currentNodeStr + " please cancel this operation and choose the update option.\n\nWould you like to continue with this operation?";
		var confirmed = confirm(txt);
		if(confirmed){
			this.selectOption(url);
		}
	}
}


function treeMenuGetNode(){
		var currentNodeId = 0;
		var currentNodeStr = '_root';
...
function COOLjsTreePRO( name, nodes, format ){
	...
	this.id='CTree_'+name;
	eval(this.id+'=this');
	if (und(window.CTrees))window.CTrees=new Array();
...
function COOLjsTreeNodePRO( treeView, parentNode , text, url, target){
    ...
    this.getContent=function( ){
		...
		function buttonSquare(node){
			...
			return node.hasChildren() ? s+'<a onclick="this.blur()"'+onm+' href="javascript:window.CTrees[\''+node.treeView.name+'\'].expandNode('+node.index+')">'+i+'</a></td>\n' : s+i+'</td>\n';
		}
		...
		var exp = 'window.CTrees[\''+this.treeView.name+'\'].expandNode('+this.index+', 0, 1)';
