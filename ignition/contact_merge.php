...
if(isset($_REQUEST['confirm']) && strtolower($_REQUEST['confirm']) == "true"){
	if($form->Validate()){
		...
		$sqlSelect = sprintf("SELECT cu.Customer_ID, c.Contact_ID, c.Is_Active, c.Is_Customer, c.Is_Supplier, p.Name_First, p.Name_Initial, p.Name_Last, o.Org_Name ");
		$sqlFrom = sprintf("FROM temp_contact AS c LEFT JOIN customer AS cu ON cu.Contact_ID=c.Contact_ID LEFT JOIN contact AS c2 ON c.Parent_Contact_ID=c2.Contact_ID LEFT JOIN person AS p ON c.Person_ID=p.Person_ID LEFT JOIN organisation AS o ON c2.Org_ID=o.Org_ID LEFT JOIN address AS a ON a.Address_ID=p.Address_ID ");
...
if(strlen($sqlSelect.$sqlFrom.$sqlWhere) > 0) {
	...
	new DataQuery(sprintf("CREATE TEMPORARY TABLE temp_results %s%s%s", mysql_real_escape_string($sqlSelect), mysql_real_escape_string($sqlFrom), $sqlWhere));
	new DataQuery(sprintf("CREATE INDEX Contact_ID ON temp_results (Contact_ID)"));

	$table = new DataTable('Search');
	$table->SetSQL(sprintf("SELECT * FROM temp_results"));
	$table->SetTotalRowSQL(sprintf("SELECT COUNT(*) AS TotalRows FROM temp_results"));
				...
				if($form2->GetValue('merge_'.$contacts[$i]['Contact_ID']) == 'Y') {
					if($_REQUEST['primary'] != $contacts[$i]['Contact_ID']) {
						if($primaryCustomer == 'Y') {
							...
							if($data->TotalRows > 0) {
								$data2 = new DataQuery(sprintf("UPDATE invoice SET Customer_ID=%d WHERE Customer_ID=%d", mysql_real_escape_string($data->Row['Customer_ID']), mysql_real_escape_string($contacts[$i]['Customer_ID'])));
								$data2 = new DataQuery(sprintf("UPDATE quote SET Customer_ID=%d WHERE Customer_ID=%d", mysql_real_escape_string($data->Row['Customer_ID']), mysql_real_escape_string($contacts[$i]['Customer_ID'])));
								$data2 = new DataQuery(sprintf("UPDATE orders SET Customer_ID=%d WHERE Customer_ID=%d", mysql_real_escape_string($data->Row['Customer_ID']), mysql_real_escape_string($contacts[$i]['Customer_ID'])));
								$data2 = new DataQuery(sprintf("UPDATE proforma SET Customer_ID=%d WHERE Customer_ID=%d", mysql_real_escape_string($data->Row['Customer_ID']), mysql_real_escape_string($contacts[$i]['Customer_ID'])));
								$data2 = new DataQuery(sprintf("UPDATE `return` SET Customer_ID=%d WHERE Customer_ID=%d", mysql_real_escape_string($data->Row['Customer_ID']), mysql_real_escape_string($contacts[$i]['Customer_ID'])));
								$data2 = new DataQuery(sprintf("UPDATE enquiry SET Customer_ID=%d WHERE Customer_ID=%d", mysql_real_escape_string($data->Row['Customer_ID']), mysql_real_escape_string($contacts[$i]['Customer_ID'])));
								$data2 = new DataQuery(sprintf("UPDATE customer_contact SET Customer_ID=%d WHERE Customer_ID=%d", mysql_real_escape_string($data->Row['Customer_ID']), mysql_real_escape_string($contacts[$i]['Customer_ID'])));
								$data2 = new DataQuery(sprintf("UPDATE customer_product SET Customer_ID=%d WHERE Customer_ID=%d", mysql_real_escape_string($data->Row['Customer_ID']), mysql_real_escape_string($contacts[$i]['Customer_ID'])));
								$data2 = new DataQuery(sprintf("UPDATE customer_product_group SET customerId=%d WHERE customerId=%d", mysql_real_escape_string($data->Row['Customer_ID']), mysql_real_escape_string($contacts[$i]['Customer_ID'])));
								$data2 = new DataQuery(sprintf("UPDATE customer_session SET Customer_ID=%d WHERE Customer_ID=%d", mysql_real_escape_string($data->Row['Customer_ID']), mysql_real_escape_string($contacts[$i]['Customer_ID'])));
								$data2 = new DataQuery(sprintf("UPDATE customer_session SET Affiliate_ID=%d WHERE Affiliate_ID=%d", mysql_real_escape_string($data->Row['Customer_ID']), mysql_real_escape_string($contacts[$i]['Customer_ID'])));
								$data2 = new DataQuery(sprintf("UPDATE customer_session_item SET Customer_ID=%d WHERE Customer_ID=%d", mysql_real_escape_string($data->Row['Customer_ID']), mysql_real_escape_string($contacts[$i]['Customer_ID'])));
								$data2 = new DataQuery(sprintf("UPDATE discount_banding_basket SET Customer_ID=%d WHERE Customer_ID=%d", mysql_real_escape_string($data->Row['Customer_ID']), mysql_real_escape_string($contacts[$i]['Customer_ID'])));
								$data2 = new DataQuery(sprintf("UPDATE discount_customer SET Customer_ID=%d WHERE Customer_ID=%d", mysql_real_escape_string($data->Row['Customer_ID']), mysql_real_escape_string($contacts[$i]['Customer_ID'])));
							}
							$data->Disconnect();
						}
