...
$page = new Page(sprintf('<a href="supplier_order_details.php?orderid=%s">[#%d] Order Details</a> &gt; Backorder %s', $order->ID, $order->ID, $ol->Product->Name), 'Backordering will mark your order lines as awaiting stock.');
...
echo $webForm->AddRow('', sprintf('<input type="button" name="back" value="back" class="btn" onclick="window.self.location.href=\'supplier_order_details.php?orderid=%d\';" /> <input type="submit" name="add" value="add" class="btn" tabindex="%s" />', $order->ID, $form->GetTabIndex()));
echo $webForm->Close();
