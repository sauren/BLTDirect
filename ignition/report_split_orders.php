...
function start(){
	...
	$page = new Page('Split Orders Report', 'Please choose a start and end date for your report');
	...
	$window = new StandardWindow("Report on Split Orders.");
...
function report($start, $end){
	...
	$data = new DataQuery(sprintf("SELECT COUNT(DISTINCT ol.Despatch_ID) AS Count FROM orders AS o INNER JOIN order_line AS ol ON o.Order_ID=ol.Order_ID WHERE o.Created_On>='%s' AND o.Created_On<'%s' AND o.Status NOT IN ('Cancelled', 'Incomplete', 'Unauthenticated') GROUP BY o.Order_ID", mysql_real_escape_string($start), mysql_real_escape_string($end)));
	while($data->Row) {
		if(!isset($despatches[$data->Row['Count']])) {
			$despatches[$data->Row['Count']] = 0;
		}

		$despatches[$data->Row['Count']]++;
	...
	$page = new Page('Schedule Breakdown Report: ' . cDatetime($start, 'longdatetime') . ' to ' . cDatetime($end, 'longdatetime'), '');
	$page->Display('header');
	?>

	<br />
	<h3>Split Pack Occurrences</h3>
	<p>Listing the accumulated totals of the number of despatches/packages for single orders.</p>

	<table width="100%" border="0" >
		<tr>
			<td style="border-bottom:1px solid #aaaaaa"><strong>Packages</strong></td>
			<td style="border-bottom:1px solid #aaaaaa"><strong>Frequency</strong></td>
		</tr>

		<?php
		if(count($despatches) > 0) {
			foreach($despatches as $packages=>$despatchItem) {
				?>

				<tr class="dataRow" onMouseOver="setClassName(this, 'dataRowOver');" onMouseOut="setClassName(this, 'dataRow');">
					<td><?php echo $packages; ?></td>
					<td><?php echo $despatchItem; ?></td>
				</tr>
