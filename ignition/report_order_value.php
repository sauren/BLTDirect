...
function start(){
	...
	$page = new Page('Order Value Report', 'Please choose a start and end date for your report');
...
$page->Display('header');
	...
	$window = new StandardWindow("Report on Order Values.");
...
function report($start, $end, $prefix, $discount = 0){
	...
	$page = new Page('Order Value Report: ' . cDatetime($start, 'longdatetime') . ' to ' . cDatetime($end, 'longdatetime'), '');
	$page->AddToHead('<script language="javascript" type="text/javascript" src="js/HttpRequest.js"></script>');
	$page->AddToHead('<script language="javascript" src="js/scw.js" type="text/javascript"></script>');
$page->Display('header');
	?>

	<h3>Order Value Count</h3>
	<p>The number of orders whose total value for the period lies between the below predefined ranges. Please note this report is based on order sub totals.</p>
	...
	$data = new DataQuery(sprintf("CREATE TEMPORARY TABLE temp_average SELECT o.Order_ID, o.SubTotal, ol.Order_Line_ID, (sp.Cost*ol.Quantity) AS Cost, ((pp.Price_Base_Our-(ol.Line_Discount/ol.Quantity))*ol.Quantity) AS Price
										FROM orders AS o
	...
	$data = new DataQuery(sprintf("CREATE INDEX Order_ID ON temp_average (Order_ID)"));
	$data->Disconnect();

	$data = new DataQuery(sprintf("CREATE INDEX SubTotal ON temp_average (SubTotal)"));
	$data->Disconnect();
	...
	foreach($ranges as $key => $range) {
		$data = new DataQuery(sprintf("SELECT COUNT(DISTINCT Order_ID) AS Count FROM temp_average WHERE SubTotal>%d AND SubTotal<=%d", $lastRange, $key));
		...
		$data = new DataQuery(sprintf("SELECT * FROM temp_average WHERE SubTotal>%d AND SubTotal<=%d", mysql_real_escape_string($lastRange), $key));
		while($data->Row) {
			...
			if(!isset($used[$uniqueName])) {
				...
				$profit[$key]['Total'] += $data->Row['SubTotal'];
				$profit[$key]['Count']++;
			}
			...
			if(!isset($usedOrders[$data->Row['Order_ID']])) {
				...
				$totalOrderValue += $data->Row['SubTotal'];
			}
	...
	<table width="100%" border="0" >
		<tr>
			...
			<td align="left" style="border-bottom:1px solid #aaaaaa" align="left"><strong>Order Value Frequency</strong></td>
			<td align="left" style="border-bottom:1px solid #aaaaaa" align="left"><strong>Order Value Percentage</strong></td>
			<td align="left" style="border-bottom:1px solid #aaaaaa" align="left"><strong>Total Order Value</strong></td>
			<td align="left" style="border-bottom:1px solid #aaaaaa" align="left"><strong>Total Order Percentage</strong></td>
			<td align="left" style="border-bottom:1px solid #aaaaaa" align="left"><strong>Average Profit</strong></td>
			<?php
			if($discount > 0) {
				?>
				<td align="left" style="border-bottom:1px solid #aaaaaa" align="left"><strong>Total Discount Value</strong></td>
				<td align="left" style="border-bottom:1px solid #aaaaaa" align="left"><strong>Average Discount</strong></td>
				<?php
			}
			?>
		</tr>
		...
		foreach($ranges as $key => $range) {
			...
			<tr class="dataRow" onMouseOver="setClassName(this, 'dataRowOver');" onMouseOut="setClassName(this, 'dataRow');">
				...
				<td align="left">&pound;<?php print number_format($profit[$key]['Total'], 2, '.', ','); ?></td>
				<td align="left"><?php print number_format(($profit[$key]['Total']/$totalOrderValue)*100, 2, '.', ','); ?>%</td>
				<td align="left">&pound;<?php print number_format(($profit[$key]['Profit']/$profit[$key]['Count']), 2, '.', ','); ?></td>
				<?php
				if($discount > 0) {
					?>
					<td align="left">&pound;<?php print number_format(($profit[$key]['Total']/100)*$discount, 2, '.', ','); ?></td>
					<td align="left">&pound;<?php print number_format((($profit[$key]['Total']/100)*$discount)/$range, 2, '.', ','); ?></td>
					<?php
				}
				?>
			</tr>
		...
		<tr class="dataRow" onMouseOver="setClassName(this, 'dataRowOver');" onMouseOut="setClassName(this, 'dataRow');">
			...
			<td align="left"></td>
			<td align="left"></td>
			<td align="left"><strong>&pound;<?php print number_format($totalOrderValue, 2, '.', ','); ?></strong></td>
			<td align="left"></td>
			<td align="left"></td>
			<?php
			if($discount > 0) {
				?>
				<td align="left"><strong>&pound;<?php print number_format(($totalOrderValue/100) * $discount, 2, '.', ','); ?></strong></td>
				<td align="left"></td>
				<?php
			}
			?>
