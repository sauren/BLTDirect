...
function start() {
	...
	$page = new Page('Catalogue Despatches Report', 'Please choose a start and end date for your report');
	...
	$window = new StandardWindow("Report on Catalogues Despatched.");
...
function report() {
	...
	$page = new Page('Catalogue Despatches Report: ' . cDatetime($form->GetValue('start'), 'longdatetime') . ' to ' . cDatetime($form->GetValue('end'), 'longdatetime'), '');
	$page->Display('header');
	?>
	...
	<table width="100%" border="0">
		...
		<?php
		$data = new DataQuery(sprintf("SELECT u.User_ID, CONCAT_WS(' ', p.Name_First, p.Name_Last) AS User, SUM(ol.Quantity) AS Quantity FROM order_line AS ol INNER JOIN orders AS o ON o.Order_ID=ol.Order_ID AND o.Created_On BETWEEN '%s' AND '%s' INNER JOIN users AS u ON u.User_ID=o.Created_By INNER JOIN person AS p ON p.Person_ID=u.Person_ID WHERE ol.Product_Title LIKE 'BLT Direct Catalogue' GROUP BY o.Created_By ORDER BY User ASC", mysql_real_escape_string($form->GetValue('start')), mysql_real_escape_string($form->GetValue('end'))));
		while($data->Row) {
			?>

			<tr class="dataRow" onMouseOver="setClassName(this, 'dataRowOver');" onMouseOut="setClassName(this, 'dataRow');">
				<td><?php echo $data->Row['User']; ?></td>
