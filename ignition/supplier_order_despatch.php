...
function step1(){
	...
	for($i=0; $i<count($order->Line); $i++){
		...
		if(empty($order->Line[$i]->DespatchID) && (($order->Line[$i]->DespatchedFrom->ID == $warehouseId) || (($order->Line[$i]->DespatchedFrom->Type == 'S') && ($order->Line[$i]->DespatchedFrom->Contact->IsDropShipper == 'N') && ($order->Line[$i]->DespatchedFrom->Contact->DropShipperID == $warehouse->ID))) && ($order->Line[$i]->Status != 'Cancelled')) {
...
function step3() {
	...
	$data = new DataQuery(sprintf("SELECT Type_Reference_ID FROM warehouse WHERE Warehouse_ID=%d AND `Type`='S'", mysql_real_escape_string($warehouseId)));
	if($data->TotalRows > 0){
		...
		if(empty($order->Line[$i]->DespatchID) && ($order->Line[$i]->Status != 'Cancelled')){
			if($_REQUEST['line' . $order->Line[$i]->ID] > 0){
				...
				if($order->Line[$i]->Quantity > $_REQUEST['line' . $order->Line[$i]->ID]){
					...
					$line->DespatchedFrom->Get($warehouseId);
				...
				if($generatePurchase){
					...
					if($purchaseLine->Product->ID > 0) {
						$data = new DataQuery(sprintf("SELECT Shelf_Location FROM warehouse_stock WHERE Warehouse_ID=%d AND Product_ID=%d AND Shelf_Location<>'' LIMIT 0, 1", mysql_real_escape_string($warehouseId), mysql_real_escape_string($purchaseLine->Product->ID)));
	...
	*/
	...
	$redirect .= '&warehouseid='.$warehouseId;
...
function step4() {
	...
	$Options = new DataQuery(sprintf("SELECT * FROM warehouse WHERE Warehouse_ID=%d", mysql_real_escape_string($warehouseId)));
	?>
