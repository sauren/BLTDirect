...
function start(){
	...
	$page = new Page('Next Day Delivery Orders Report', 'Please choose a start and end date for your report');
...
function report() {
	...
	$page = new Page('Next Day Delivery Orders Report: ' . cDatetime($form->GetValue('start'), 'longdatetime') . ' to ' . cDatetime($form->GetValue('end'), 'longdatetime'), '');
	$page->Display('header');
	?>

	<br />
	<h3>Telesales Orders</h3>
	<p>Listing telesales orders with next day delivery postage option where more than one despatches have been made.</p>
	...
	<table width="100%" border="0">
		<tr>
			...
			<td style="border-bottom: 1px solid #aaaaaa;"><strong>Postage</strong></td>
			<td style="border-bottom: 1px solid #aaaaaa;"><strong>Despatches</strong></td>
			<td style="border-bottom: 1px solid #aaaaaa;"><strong>Supplier Settings</strong></td>
			<td style="border-bottom: 1px solid #aaaaaa;" align="right"><strong>Supplier Cost</strong></td>
		</tr>

		<?php
		$data = new DataQuery(sprintf("SELECT o.Order_Prefix, o.Order_ID, o.Created_On, p.Postage_Title, COUNT(DISTINCT d.Despatch_ID) AS DespatchCount FROM orders AS o INNER JOIN postage AS p ON p.Postage_ID=o.Postage_ID AND Postage_Days=1 INNER JOIN order_line AS ol ON ol.Order_ID=o.Order_ID INNER JOIN despatch AS d ON d.Despatch_ID=ol.Despatch_ID WHERE o.Order_Prefix='T' AND o.Created_On>'%s' AND o.Created_On<'%s' GROUP BY o.Order_ID HAVING DespatchCount>1 ORDER BY DespatchCount DESC", mysql_real_escape_string($form->GetValue('start')), mysql_real_escape_string($form->GetValue('end'))));
		if($data->TotalRows > 0) {
			while($data->Row) {
				$order = new Order($data->Row['Order_ID']);
				$order->GetLines();
                $order->GetSupplierShipped();

                $isShippingComplete = true;
                $shippingTotal = 0;

			    if(count($order->SupplierShipped) > 0) {
                    for($i=0; $i<count($order->SupplierShipped); $i++) {
                    	if($order->SupplierShipped[$i]->HasErrors) {
							$isShippingComplete = false;
						} else {
							$shippingTotal += $order->SupplierShipped[$i]->GetTotal();
						}
					}
				}
				?>
				...
				<tr>
					...
					<td><?php echo $data->Row['DespatchCount']; ?></td>
					<td><?php echo ($isShippingComplete) ? '' : 'Incomplete'; ?></td>
					<td align="right">&pound;<?php echo number_format(round($shippingTotal, 2), 2, '.', ','); ?></td>
				</tr>
