...
if($adminAccess == true){
	$form->AddField('person', 'Timesheet For', 'select', '', 'anything', 1, 100);
	foreach ($userNames as $userName) {
		$uName = $userName["First_Name"]." ".$userName["Last_Name"];
		if(isset($userName["First_Name"]) || ($userName["First_Name"])){
			$form->AddOption('person', $userName["Person_ID"], $uName);
		}
	}
}

//$form->AddField('id','ID','hidden', $userName["Product_ID"], 'anything', 1,11);
...
if(isset($_REQUEST['confirm']) && (strtolower($_REQUEST['confirm']) == 'true')) {
	if($form->Validate()) {
		if(strlen($form->GetValue('enddate')) == 0) {
			...
			if(!$form->GetValue('person')){
				$timesheet->User->ID = $user->ID;
			}else{
				$timesheet->User->ID = $form->GetValue('person');
			}
...
echo $webForm->Open();
if($adminAccess == true){
	echo $webForm->AddRow($form->GetLabel('person'), $form->GetHTML('person') . $form->GetIcon('person'));
}else{
