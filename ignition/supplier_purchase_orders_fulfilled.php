...
function view($supplierId) {
    $page = new Page('Fulfilled Purchase Orders', 'Listing all fulfilled purchase orders.');
	$page->Display('header');

	$table = new DataTable('purchaseorders');
	$table->SetSQL(sprintf("SELECT *, DATE(Created_On) AS CreatedDate FROM purchase WHERE Supplier_ID=%d AND For_Branch>0 AND Purchase_Status LIKE 'Fulfilled'", $supplierId));
