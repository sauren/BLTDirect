...
function view(){
	...
	$page = new Page('New/Pending Action Enquiries', 'Below is a list of enquiries which are pending action.');
	$page->Display('header');

	$sqlType = (isset($_REQUEST['type'])) ? sprintf(" AND et.Developer_Key LIKE '%s'", $_REQUEST['type']) : '';

	$table = new DataTable('enquiries');
	$table->SetSQL(sprintf("SELECT e.*, et.Name, o.Org_Name, p.Name_First, p.Name_Last, p2.Name_First AS Owner_First, p2.Name_Last AS Owner_Last FROM enquiry AS e LEFT JOIN enquiry_type AS et ON et.Enquiry_Type_ID=e.Enquiry_Type_ID INNER JOIN customer AS c ON c.Customer_ID=e.Customer_ID INNER JOIN contact AS n ON n.Contact_ID=c.Contact_ID INNER JOIN person AS p ON p.Person_ID=n.Person_ID LEFT JOIN contact AS n2 ON n.Parent_Contact_ID=n2.Contact_ID LEFT JOIN organisation AS o ON o.Org_ID=n2.Org_ID LEFT JOIN users AS u ON u.User_ID=e.Owned_By LEFT JOIN person AS p2 ON p2.Person_ID=u.Person_ID WHERE e.Status NOT LIKE 'Closed' AND e.Review_On='0000-00-00 00:00:00' AND (e.Owned_By=%d OR e.Owned_By=0) %s GROUP BY e.Enquiry_ID", mysql_real_escape_string($GLOBALS['SESSION_USER_ID']), mysql_real_escape_string($sqlType)));
	$table->SetMaxRows(25);
	...
	<table align="center" cellpadding="4" cellspacing="0" class="DataTable">
		<thead>
			<tr>
				<th nowrap="nowrap" class="dataHeadOrdered">Enquired On</th>
			...
			if($data->TotalRows > 0) {
				while($data->Row) {
					...
					<tr class="dataRow" onMouseOver="setClassName(this, 'dataRowOver');" onMouseOut="setClassName(this, 'dataRow');">
						<td class="dataOrdered" align="left" <?php echo ($data->Row['Is_Big_Enquiry'] == 'Y') ? 'style="background-color: #FF9D9D;"' : (($data->Row['Is_Trade_Enquiry'] == 'Y') ? 'style="background-color: #FEFC6B;"' : ''); ?>><?php echo $data->Row['Modified_On']; ?></td>
