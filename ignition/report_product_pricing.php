...
function start(){
	...
	$page = new Page('Product Pricing Report', 'Please choose a start and end date for your report');
...
$page->Display('header');
	...
	$window = new StandardWindow("Report on Product Pricing.");
	...
	echo $window->Open();
	echo $window->AddHeader('Select a product to report pricing on.');
...
function report($start, $end, $pid){
	...
	$page = new Page('Product Pricing Report : ' . cDatetime($start, 'longdatetime') . ' to ' . cDatetime($end, 'longdatetime'), '');
	$page->AddToHead('<script language="javascript" src="js/scw.js" type="text/javascript"></script>');
$page->Display('header');
	?>
	<br />
	<h3><?php print $product->Name; ?> [#<?php print $product->ID; ?>] Pricing</h3>
	<p>Listing product pricing information for the given period.</p>

	<?php
	$sales = array();
	$totalSold = 0;

	$data = new DataQuery(sprintf("SELECT o.Created_On, ol.Price, ol.Quantity FROM orders AS o INNER JOIN order_line AS ol ON ol.Order_ID=o.Order_ID WHERE ol.Product_ID=%d AND o.Created_On BETWEEN '%s' AND '%s' ORDER BY o.Created_On ASC", mysql_real_escape_string($product->ID), $start, $end));
	while($data->Row) {
		if(!isset($sales[$data->Row['Price']])) {
			$sales[$data->Row['Price']]['start'] = $data->Row['Created_On'];
			$sales[$data->Row['Price']]['quantity'] = 0;
		}

		$sales[$data->Row['Price']]['end'] = $data->Row['Created_On'];
		$sales[$data->Row['Price']]['quantity'] += $data->Row['Quantity'];

		$totalSold += $data->Row['Quantity'];
	...
	<table width="100%" border="0">
	  <tr>
		<td style="border-bottom:1px solid #aaaaaa" align="left"><strong>Price Period</strong></td>
		<td style="border-bottom:1px solid #aaaaaa" align="right"><strong>Sale Price</strong></td>
		<td style="border-bottom:1px solid #aaaaaa" align="center"><strong>Quantity Sold</strong></td>
		<td style="border-bottom:1px solid #aaaaaa" align="left"><strong>Percentage</strong></td>
	  </tr>

	<?php
	foreach($sales as $price=>$sale) {
		?>

	  	<tr class="dataRow" onMouseOver="setClassName(this, 'dataRowOver');" onMouseOut="setClassName(this, 'dataRow');">
			<td align="left"><?php echo cDatetime($sale['start'], 'longdatetime'); ?> - <?php echo cDatetime($sale['end'], 'longdatetime'); ?></td>
			<td align="right">&pound;<?php echo number_format($price, 2, '.', ','); ?></td>
			<td align="center"><?php echo $sale['quantity']; ?></td>
			<td align="left"><?php echo number_format(($sale['quantity']/$totalSold)*100, 2, '.', ''); ?>%</td>
		</tr>
