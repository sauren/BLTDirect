...
function start(){
	...
	$page = new Page('Order Type Report', 'Please choose a start and end date for your report');
	...
	$form->AddField('conversion', 'Conversions', 'select', '', 'alpha_numeric', 0, 32);
	$form->AddOption('conversion', '', '-- None --');
	$form->AddOption('conversion', 'WW', 'Web -> Web');
	$form->AddOption('conversion', 'WT', 'Web -> Telesales');
	$form->AddOption('conversion', 'TT', 'Telesales -> Telesales');
	$form->AddOption('conversion', 'TW', 'Telesales -> Web');

	if(isset($_REQUEST['confirm']) && strtolower($_REQUEST['confirm']) == "true"){
		if(($form->GetValue('range') != 'none') && (strlen($form->GetValue('range')) > 1)) {
			if(strlen($form->GetValue('conversion')) == 0){
				$form->AddError('Conversion must have a selected value. Please try again.', 'conversion');
			}
			if($form->Valid){
				...
				report($start, $end, $form->GetValue('conversion'));
				exit;
			}
		} else {
...
$page->Display('header');
	...
	$window = new StandardWindow("Report on Order Types.");
	...
	echo $window->Open();
	echo $window->AddHeader('Select a conversion order type to report on.');
	echo $window->OpenContent();
	echo $webForm->Open();
	echo $webForm->AddRow($form->GetLabel('conversion'), $form->GetHTML('conversion'));
	echo $webForm->Close();
...
function report($start, $end, $conversion){
	...
	$from = substr($conversion, 0, 1);
	$to = substr($conversion, 1, 1);

	$page = new Page('Order Type Report : ' . cDatetime($start, 'longdatetime') . ' to ' . cDatetime($end, 'longdatetime'), '');
	$page->AddToHead('<script language="javascript" src="js/scw.js" type="text/javascript"></script>');
$page->Display('header');
	?>
	<br />
	<h3>Order Conversions</h3>
	<p>Listing number of orders per customer from type <strong><?php print $orderTypes[$from]; ?></strong> to <strong><?php print $orderTypes[$to]; ?></strong>. Each customer must have at least two orders for inclusion of this report.</p>

	<?php
	$converts = array();
	$customers = array();

	$data = new DataQuery(sprintf("SELECT o.Order_Prefix, o.Customer_ID FROM orders AS o WHERE (o.Created_On BETWEEN '%s' AND '%s') AND (o.Order_Prefix LIKE '%s' OR o.Order_Prefix LIKE '%s') ORDER BY o.Created_On ASC", $start, $end, mysql_real_escape_string($from), mysql_real_escape_string($to)));
	while($data->Row) {
		if(!isset($converts[$data->Row['Customer_ID']])) {
			if($from == $data->Row['Order_Prefix']) {
				$converts[$data->Row['Customer_ID']] = true;
			} else {
				$converts[$data->Row['Customer_ID']] = false;
			}
		} else {
			if($converts[$data->Row['Customer_ID']]) {
				if($to == $data->Row['Order_Prefix']) {
					$converts[$data->Row['Customer_ID']] = false;
					$customers[] = $data->Row['Customer_ID'];
				}
			}
		}
	...
	echo 'Number of customers: ' . count($customers);
