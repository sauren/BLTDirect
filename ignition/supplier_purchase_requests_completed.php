...
function view($supplierId) {
    $page = new Page('Completed Purchase Requests', 'Listing all completed purchase requests.');
	$page->Display('header');

	$table = new DataTable('purchaserequests');
	$table->SetSQL(sprintf("SELECT *, DATE(CreatedOn) AS CreatedDate FROM purchase_request WHERE Status LIKE 'Completed' AND SupplierID=%d", $supplierId));
