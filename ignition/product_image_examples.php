...
function remove(){
	if(isset($_REQUEST['confirm']) && ($_REQUEST['confirm'] == 'true') && isset($_REQUEST['img'])){
		$image = new ProductImageExample;
		$image->Delete($_REQUEST['img']);
	}

	redirect(sprintf("Location: product_image_examples.php?pid=%d", $_REQUEST['pid']));
}
...
function add(){
	...
	$data = new DataQuery(sprintf("SELECT COUNT(*) AS Count FROM product_image_example WHERE Product_ID=%d", mysql_real_escape_string($_REQUEST['pid'])));
	...
	if(isset($_REQUEST['confirm'])) {
		if($form->Validate()){
			...
			if($image->Add($thumbField, $largeField)) {
		   		redirect(sprintf("Location: product_image_examples.php?pid=%d", $form->GetValue('pid')));
			} else {
	...
	$page = new Page(sprintf('<a href="product_profile.php?pid=%s">Product Profile</a> &gt; <a href="product_image_examples.php?pid=%s">Product Image Examples</a> &gt; Add Product Image Example', $_REQUEST['pid'], $_REQUEST['pid']),'The more information you supply the better your system will become');
	$page->Display('header');
	...
	$window = new StandardWindow("Add a Product Image Example.");
	...
	echo $webForm->AddRow("&nbsp;", sprintf('<input type="button" name="back" value="back" class="btn" onClick="window.self.location=\'product_image_examples.php?pid=%s\';"> <input type="submit" name="add" value="add" class="btn" tabindex="%s">', $_REQUEST['pid'], $form->GetTabIndex()));
	echo $webForm->Close();
...
function update(){
	...
	$page = new Page(sprintf('<a href="product_profile.php?pid=%s">Product Profile</a> &gt; <a href="product_image_examples.php?pid=%s">Product Image Examples</a> &gt; Update Product Image Example', $_REQUEST['pid'], $_REQUEST['pid']),'The more information you supply the better your system will become');
	$page->Display('header');
	...
	$window = new StandardWindow("Update a Product Image Example.");
	$webForm = new StandardForm;
	...
	echo $webForm->AddRow($form->GetLabel('active'), $form->GetHTML('active') . $form->GetIcon('active'));
	echo $webForm->AddRow($form->GetLabel('default'), $form->GetHTML('default') . $form->GetIcon('default'));
	echo $webForm->AddRow("&nbsp;", sprintf('<input type="button" name="back" value="back" class="btn" onClick="window.self.location=\'product_image_examples.php?pid=%s\';"> <input type="submit" name="update" value="update" class="btn" tabindex="%s">', $_REQUEST['pid'], $form->GetTabIndex()));
	echo $webForm->Close();
	echo $window->CloseContent();
	echo $window->Close();
	echo $form->Close();
...
function view(){
	...
	$page = new Page(sprintf('<a href="product_profile.php?pid=%s">Product Profile</a> &gt; Product Image Examples', $_REQUEST['pid']),'You may add more than one image to your Products, however only one may be the default image used by your Product Categories. Click on a thumnail image to view larger images in a new browser window.');
	$page->Display('header');

	$data = new DataQuery(sprintf("select * from product_image_example where Product_ID=%d", mysql_real_escape_string($_REQUEST['pid'])));
	if($data->TotalRows == 0){
		...
		while($data->Row){
			echo sprintf('<tr><td><a href="%s%s" target="_blank"><img src="%s%s" border="0"/></a></td><td>%s&nbsp;</td><td>%s</td><td>%s</td><td width="1%%" nowrap="nowrap"><a href="product_image_examples.php?action=update&img=%s&pid=%s"><img src="./images/icon_edit_1.gif" alt="Update Settings" border="0"></a> <a href="javascript:confirmRequest(\'product_image_examples.php?action=remove&confirm=true&img=%s&pid=%s\',\'Are you sure you want to remove this Image. The image will be deleted on the server.\');"><img src="./images/aztector_6.gif" alt="Remove" border="0"></a></td></tr>',
							$GLOBALS['PRODUCT_EXAMPLE_IMAGES_DIR_WS'],
							$data->Row['Image_Src'],
							$GLOBALS['PRODUCT_EXAMPLE_IMAGES_DIR_WS'],
							...
							$data->Row['Is_Primary'],
							$data->Row['Is_Active'],
							$data->Row['Product_Image_Example_ID'],
							$data->Row['Product_ID'],
							$data->Row['Product_Image_Example_ID'],
							$data->Row['Product_ID']);
			$data->Next();
	...
	echo "<br>";
	echo sprintf('<input type="button" name="add" value="add a new example" class="btn" onclick="window.location.href=\'product_image_examples.php?action=add&pid=%d\'">', $_REQUEST['pid']);
