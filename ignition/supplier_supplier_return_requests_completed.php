...
function view($supplierId) {
    $page = new Page('Completed Supplier Return Requests', 'Listing all completed supplier return requests.');
	$page->Display('header');

	$table = new DataTable('requests');
	$table->SetSQL(sprintf("SELECT srr.*, DATE(srr.CreatedOn) AS CreatedDate FROM supplier_return_request AS srr WHERE srr.Status LIKE 'Completed' AND srr.SupplierID=%d", $supplierId));
