...
function open() {
	...
	<html>
	<head>
		<title>Catalogue Requests Print Preview</title>
		<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
	</head>

	<frameset border="0" rows="60,*" framespacing="2" frameborder="yes" id="frameSet" bordercolor="#5C6B80">
		<frame src="?action=openheader" name="frameHeader" frameborder="no" scrolling="auto" border="0"></frame>
		<frame src="?action=openbody" name="frameBody" frameborder="no" scrolling="auto" border="0"></frame>
	</frameset>
...
function openHeader() {
	...
	<body style="padding-bottom: 0px;">
		...
		<table width="100%">
			<tr>
				<td width="50%" align="left"><span class="pageTitle">Catalogue Requests Print Preview</span></td>
				<td width="50%" align="right"><a href="javascript:popUrl('?action=openbody&print=true' , 800, 600);"><img src="images/icon_print_1.gif" border="0" width="16" height="16" alt="Print" /></a></td>
			</tr>
		</table>
...
function openBody() {
	...
	while($data->Row) {
		...
		if(isset($_REQUEST['print']) && ($_REQUEST['print'] == 'true')) {
			$contact = new Contact($data->Row['Contact_ID']);
			$contact->CatalogueSentOn = date('Y-m-d H:i:s');
			$contact->Update();

			$findReplace = new FindReplace();
			$findReplace->Add('/\[NAME\]/', $contact->Person->GetFullName());
			$findReplace->Add('/\[BODY\]/', Template::GetContent('email_catalogue_request'));
		
			$emailBody = $findReplace->Execute(Template::GetContent('email_template_standard'));

			$queue = new EmailQueue();
			$queue->GetModuleID('contacts');
			$queue->Subject = sprintf("%s Catalogue Request", $GLOBALS['COMPANY']);
