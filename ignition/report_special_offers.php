...
function start(){
	...
	$page = new Page('Special Offers Report', 'Please choose a start and end date for your report');
...
$page->Display('header');
	...
	$window = new StandardWindow("Report on Special Offers.");
...
function report($start, $end){
	...
	$page = new Page('Special Offers Report : ' . cDatetime($start, 'longdatetime') . ' to ' . cDatetime($end, 'longdatetime'), '');
	$page->Display('header');
	?>

	  <h3>Special Offer Orders</h3>
	<p>Orders containing product lines associated to special offers.</p>
	<table width="100%" border="0">
	  ...
	  <?php
	  $data = new DataQuery(sprintf("SELECT DISTINCT(o.Order_ID), o.Created_On, o.Total FROM orders AS o INNER JOIN order_line AS l ON l.Order_ID=o.Order_ID INNER JOIN product AS p ON l.Product_ID=p.Product_ID INNER JOIN product_offers AS f ON f.Product_ID=p.Product_ID WHERE (o.Created_On BETWEEN f.Offer_Start_On AND f.Offer_End_On) AND (o.Created_On BETWEEN '%s' AND '%s') AND o.Status NOT IN ('Cancelled', 'Incomplete', 'Unauthenticated') ORDER BY o.Order_ID DESC", mysql_real_escape_string($start), mysql_real_escape_string($end)));
	  $counter = 1;
	  while($data->Row) {
			?>
			  <tr class="dataRow" onMouseOver="setClassName(this, 'dataRowOver');" onMouseOut="setClassName(this, 'dataRow');">
				...
				<td align="right"><a href="order_details.php?orderid=<?php echo $data->Row['Order_ID']; ?>"><?php echo $data->Row['Order_ID']; ?></a></td>
