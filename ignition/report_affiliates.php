...
function start(){
	...
	$page = new Page('Affiliate Report', 'Please choose a start and end date for your report');
...
$page->Display('header');
	...
	$window = new StandardWindow("Report on Affiliates.");
...
function report($start, $end){
	...
	$page = new Page('Affiliate Report : ' . cDatetime($start, 'longdatetime') . ' to ' . cDatetime($end, 'longdatetime'), '');
	$page->AddToHead('<script language="javascript" src="js/scw.js" type="text/javascript"></script>');
$page->Display('header');
	...
	<h3><br />
	Affiliates</h3>
	<p>The number of affiliates and click throughs resulting in orders.</p>
	<table width="100%" border="0">
	  <tr>
		<td style="border-bottom:1px solid #aaaaaa"><strong>Affiliate Name</strong></td>
		<td style="border-bottom:1px solid #aaaaaa" align="right"><strong>Click Throughs</strong></td>
		<td style="border-bottom:1px solid #aaaaaa" align="right"><strong>Sales</strong></td>
	  </tr>
	<?php
	$affiliates = array();
	
	$data = new DataQuery(sprintf("SELECT Customer_ID FROM customer WHERE Is_Affiliate='Y'"));
	while($data->Row) {
		$affiliates[$data->Row['Customer_ID']] = array();
		$affiliates[$data->Row['Customer_ID']]['clicks'] = 0;
		$affiliates[$data->Row['Customer_ID']]['sales'] = 0;	
		
		$data->Next();
	}
	$data->Disconnect();

	$data = new DataQuery(sprintf("SELECT Affiliate_ID, COUNT(*) AS Count FROM customer_session WHERE Affiliate_ID>0 AND Created_On BETWEEN '%s' AND '%s' GROUP BY Affiliate_ID", $start, $end));
	while($data->Row) {
		if(isset($affiliates[$data->Row['Affiliate_ID']])) {
			$affiliates[$data->Row['Affiliate_ID']]['clicks'] += $data->Row['Count'];
		}
	...
	$data = new DataQuery(sprintf("SELECT Affiliate_ID, COUNT(*) AS Count, SUM(SubTotal) AS SubTotal FROM orders WHERE Affiliate_ID>0 AND Created_On BETWEEN '%s' AND '%s' AND Status NOT IN ('Cancelled', 'Incomplete', 'Unauthenticated') GROUP BY Affiliate_ID", $start, $end));
	while($data->Row) {
		if(isset($affiliates[$data->Row['Affiliate_ID']])) {
			$affiliates[$data->Row['Affiliate_ID']]['sales'] += $data->Row['Count'];
		}
	...
	foreach($affiliates as $key => $value) {
		...
		<td><?php echo sprintf('%s %s %s', $customer->Contact->Person->Title, $customer->Contact->Person->Name, $customer->Contact->Person->LastName); ?></td>
		<td align="right"><?php echo $value['clicks']; ?></td>
