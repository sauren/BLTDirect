...
<head>
	...
	<title>BLT Direct Privacy Policy</title>
	<!-- InstanceEndEditable -->
        ...
        <div id="PageWrapper">
            <div id="Page">
                <div id="PageContent">
                    ...
                    <?php
                    $data = new DataQuery(sprintf("SELECT * FROM article WHERE Article_Category_ID=%d AND Is_Active='Y' ORDER BY Created_On DESC", PRIVACY_ARTICLE));

                        while($data->Row) {
                            print sprintf('<h1>%s</h1><p>%s</p>', $data->Row['Article_Title'], stripslashes($data->Row['Article_Description']));
                            $data->Next();
                        }
