...
<div id="PageWrapper">
	<div id="Page">
		<div id="PageContent"><!-- InstanceBeginEditable name="pageContent" -->
				<h1>Orders Despatched Report</h1>
				<br />
				...
				$window = new StandardWindow("Report Period");
				$webForm = new StandardForm;

				echo $window->Open();
				echo $window->AddHeader('Select date for report.');
				echo $window->OpenContent();
				echo $webForm->Open();
				echo $webForm->AddRow($form->GetLabel('date'), $form->GetHTML('date'));
				echo $webForm->AddRow('', '<input type="submit" class="submit" name="submit" value="Submit" />', $form->GetTabIndex());
				echo $webForm->Close();
				...
				if(isset($_REQUEST['confirm'])) {
					if($form->Valid) {
						$date = sprintf('%s-%s-%s 00:00:00', substr($form->GetValue('date'), 6, 4), substr($form->GetValue('date'), 3, 2), substr($form->GetValue('date'), 0, 2));
						?>
                        ...
                        <table width="100%" border="0">
							...
							<?php
							$data = new DataQuery(sprintf("SELECT * FROM purchase AS p WHERE p.Order_ID>0 AND p.Supplier_ID=%d AND p.Created_On>='%s' AND p.Created_On<ADDDATE('%s', INTERVAL 1 DAY)", mysql_real_escape_string($session->Warehouse->Contact->ID), mysql_real_escape_string($date), mysql_real_escape_string($date)));
							if($data->TotalRows > 0) {
								while($data->Row) {
									?>
				...
				echo sprintf('<input type="button" class="greySubmit" name="print" value="Print" onclick="popUrl(\'report_orders_despatched_print.php?date=%s\', 800, 600);" />', $date);
				echo $form->Close();
				?>
