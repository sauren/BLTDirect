...
<div id="PageWrapper">
	<div id="Page">
		<div id="PageContent"><!-- InstanceBeginEditable name="pageContent" -->
			<h1>Completed Purchase Requests</h1>
			<br />

			<?php
			$table = new DataTable("requests");
			$table->SetSQL(sprintf("SELECT pr.* FROM purchase_request AS pr WHERE pr.Status LIKE 'Completed' AND pr.SupplierID=%d", $supplierId));
			$table->AddField('Purchase Request ID', 'PurchaseRequestID', 'left');
			$table->AddField('Purchase Request Date', 'CreatedOn', 'left');
			$table->AddField('Status', 'Status', 'left');
			$table->AddLink("purchase_request_details.php?id=%s", "<img src=\"images/folderopen.gif\" alt=\"Open Purchase Request Details\" border=\"0\">", "PurchaseRequestID");
			$table->SetMaxRows(25);
