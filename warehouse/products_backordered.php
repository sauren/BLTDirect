...
<div id="PageWrapper">
	<div id="Page">
		...
		if($session->Warehouse->Type == 'S') {
			$sqlSelect .= sprintf("SELECT ol.*, SUM(ol.Quantity) AS Quantity ");
			$sqlFrom .= sprintf("FROM orders AS o INNER JOIN order_line AS ol ON ol.Order_ID=o.Order_ID AND ol.Despatch_ID=0 AND ol.Despatch_From_ID=%d AND ol.Line_Status LIKE 'Backordered' ", $session->Warehouse->ID);
			$sqlWhere .= sprintf("WHERE o.Is_Declined='N' AND o.Is_Failed='N' AND o.Is_Warehouse_Declined='N' AND (o.Status LIKE 'Packing' OR o.Status LIKE 'Partially Despatched') ");
			$sqlGroup .= sprintf("GROUP BY ol.Product_ID ");
		} else {
            $sqlSelect .= sprintf("SELECT ol.*, SUM(ol.Quantity) AS Quantity ");
			$sqlFrom .= sprintf("FROM orders AS o INNER JOIN order_line AS ol ON o.Order_ID=ol.Order_ID AND ol.Despatch_ID=0 INNER JOIN warehouse AS w ON ol.Despatch_From_ID=w.Warehouse_ID LEFT JOIN supplier AS s ON w.Type_Reference_ID=s.Supplier_ID AND w.Type='S' ");
        ...
        $table->AddField('Expected', 'Backorder_Expected_On', 'left');
