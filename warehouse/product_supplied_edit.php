...
$supplierProd= new SupplierProduct($_REQUEST['spid']);
$supplierProd->Get();

$form = new Form($_SERVER['PHP_SELF']);
$form->AddField('spid','spid','hidden',$_REQUEST['spid'],'numeric_unsigned',0,11);
$form->AddField('action', 'Action', 'hidden', 'register', 'alpha', 8, 8);
$form->AddField('confirm', 'Confirm', 'hidden', 'true', 'alpha', 4, 4);
$form->AddField('category', 'Category ID', 'hidden', 0, 'numeric_unsigned', 1, 11);
$form->AddField('sku', 'Your Part Number', 'text', $supplierProd->SKU, 'alpha_numeric',1,45, false);
$form->AddField('cost', 'Cost Price', 'text', $supplierProd->Cost, 'float',1,11, false);

if(isset($_REQUEST['confirm']) && strtolower($_REQUEST['confirm']) == "true"){
	if($form->Validate()) {
		$supplierProd->SKU = $form->GetValue('sku');
		$supplierProd->Cost = $form->GetValue('cost');
		$supplierProd->Update();

		redirect(sprintf("Location: products_supplied.php?category=%d", $form->GetValue('category')));
	}
}
?>
...
<div id="PageWrapper">
	<div id="Page">
		...
		if($session->Warehouse->Type == 'S'){
			if($session->Warehouse->Contact->ID == $supplierProd->Supplier->ID){
				if(!$form->Valid){
					echo $form->GetError();
					echo "<br>";
				}
				...
				echo $form->GetHTML('spid');
				echo $form->GetHTML('category');
				echo $window->Open();
				echo $window->AddHeader('Please fill in the product details details');
				...
				echo $webForm->AddRow("&nbsp;", sprintf('<input type="button" name="back" value="Back" class="greySubmit" onClick="window.self.location=\'products_supplied.php?category=%d\';"> <input type="submit" name="add" value="update" class="submit" tabindex="%s">', $form->GetValue('category'), $form->GetTabIndex()));
				echo $webForm->Close();
				echo $window->CloseContent();
				echo $window->Close();
				echo $form->Close();
			} else {
				echo "This product belongs to another supplier";
			}
		} else {
			echo "Sorry this section is for suppliers only";
		}
		?>
		<!-- InstanceEndEditable -->
		</div>
