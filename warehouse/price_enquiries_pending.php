...
<div id="PageWrapper">
	<div id="Page">
		<div id="PageContent"><!-- InstanceBeginEditable name="pageContent" -->
			<h1>Pending Price Enquiries</h1>
			<br />

			<?php
			$table = new DataTable("enquiries");
			$table->SetSQL(sprintf("SELECT pe.*, pes.Is_Complete, IF(pes.Position>0, pes.Position, '') AS Position_Text FROM price_enquiry AS pe INNER JOIN price_enquiry_supplier AS pes ON pes.Price_Enquiry_ID=pe.Price_Enquiry_ID WHERE pe.Status LIKE 'Pending' AND pes.Supplier_ID=%d", $supplierId));
