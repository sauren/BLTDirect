...
<div id="PageWrapper">
	<div id="Page">
		...
		$sql = sprintf("SELECT p.Product_Title,p.SKU, w.* from warehouse_stock w
						INNER JOIN product p ON p.Product_ID = w.Product_ID
						WHERE w.Warehouse_ID = %d",$session->Warehouse->ID);
		...
		$table->AddField('Shelf Location', 'Shelf_Location', 'right');
		$table->AddLink('product_stocked_edit.php?sid=%s',"<img src=\"./images/icon_edit_1.gif\" alt=\"Update the stock settings\" border=\"0\">",'Stock_ID');
		$table->AddLink("../product.php?pid=%s",
								"<img src=\"../ignition/images/folderopen.gif\" alt=\"View Product\" border=\"0\">",
								"Product_ID");
		$table->SetMaxRows(25);
