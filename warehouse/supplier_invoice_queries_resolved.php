...
<div id="PageWrapper">
	<div id="Page">
		<div id="PageContent"><!-- InstanceBeginEditable name="pageContent" -->
			<h1>Resolved Invoice Queries</h1>
			<br />

			<?php
            $table = new DataTable('queries');
			$table->SetSQL(sprintf("SELECT siq.* FROM supplier_invoice_query AS siq WHERE siq.Status LIKE 'Resolved' AND siq.SupplierID=%d", $supplierId));
			$table->AddField('Invoice Query ID', 'SupplierInvoiceQueryID');
			$table->AddField('Invoice Query Date','CreatedOn');
			$table->AddField('Status', 'Status', 'left');
			$table->AddField('Invoice Reference', 'InvoiceReference', 'left');
			$table->AddField('Total', 'Total', 'right');
			$table->AddLink('supplier_invoice_query_details.php?id=%s', '<img src="images/folderopen.gif" alt="Open" border="0" />', 'SupplierInvoiceQueryID');
			$table->SetMaxRows(25);
