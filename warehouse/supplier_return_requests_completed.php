...
<div id="PageWrapper">
	<div id="Page">
		<div id="PageContent"><!-- InstanceBeginEditable name="pageContent" -->
			<h1>Completed Return Requests</h1>
			<br />

			<?php
			$table = new DataTable("requests");
			$table->SetSQL(sprintf("SELECT srr.* FROM supplier_return_request AS srr WHERE srr.Status LIKE 'Completed' AND srr.SupplierID=%d", $supplierId));
			$table->AddField('Return Request ID', 'SupplierReturnRequestID', 'left');
			$table->AddField('Return Request Date', 'CreatedOn', 'left');
			$table->AddField('Status', 'Status', 'left');
			$table->AddField('Authorisation', 'AuthorisationNumber', 'left');
			$table->AddField('Total', 'Total', 'right');
			$table->AddLink("supplier_return_request_details.php?id=%s", "<img src=\"images/folderopen.gif\" alt=\"Open Return Request Details\" border=\"0\">", "SupplierReturnRequestID");
			$table->SetMaxRows(25);
