...
function view() {
	...
	$data = new DataQuery(sprintf("SELECT p.Product_ID, p.Product_Title, r.quantity AS Quantity_Reserved, SUM(wr.quantity) AS Quantity_Remaining, sp.Supplier_SKU, sp.Cost FROM warehouse_reserve AS wr INNER JOIN warehouse AS w ON w.Warehouse_ID=wr.warehouseId AND w.Type='S' AND w.Type_Reference_ID=%d INNER JOIN product AS p ON p.Product_ID=wr.productId LEFT JOIN supplier_product AS sp ON sp.Product_ID=p.Product_ID AND sp.Supplier_ID=w.Type_Reference_ID LEFT JOIN (SELECT r.supplierId, ri.productId, SUM(ri.quantity-ri.quantityRemaining) AS quantity FROM reserve AS r INNER JOIN reserve_item AS ri ON ri.reserveId=r.id GROUP BY r.supplierId, ri.productId) AS r ON r.supplierId=w.Type_Reference_ID AND r.productId=p.Product_ID GROUP BY p.Product_ID ORDER BY p.Product_Title ASC", mysql_real_escape_string($session->Warehouse->Contact->ID)));
	while($data->Row) {
		$products[] = $data->Row;
		...
		<div id="PageContent"><!-- InstanceBeginEditable name="pageContent" -->
					...
					<table width="100%" border="0">
						<tr>
							...
							<td style="border-bottom:1px solid #aaaaaa;" align="right"><strong>Quantity Reserved</strong></td>
							<td style="border-bottom:1px solid #aaaaaa;" align="right"><strong>Quantity Remaining</strong></td>
						</tr>
						...
						if(!empty($products)) {
							foreach($products as $productData) {
								...
								<tr class="dataRow" onMouseOver="setClassName(this, 'dataRowOver');" onMouseOut="setClassName(this, 'dataRow');">
									...
									<td><?php echo $productData['Supplier_SKU']; ?></td>
									<td align="right">&pound;<?php echo number_format($productData['Cost'], 2, '.', ','); ?></td>
									<td align="right"><?php echo $productData['Quantity_Reserved']; ?></td>
									<td align="right"><?php echo $productData['Quantity_Remaining']; ?></td>
								</tr>
