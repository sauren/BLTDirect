...
$data = new DataQuery(sprintf("SELECT Order_Note_ID FROM order_note WHERE Order_ID=%d AND Is_Alert='Y' %s", mysql_real_escape_string($order->ID), ($session->Warehouse->Type == 'S') ? "AND Is_Public='Y'" : ''));
while($data->Row){
...
$data = new DataQuery(sprintf("SELECT Order_Warehouse_Note_ID FROM order_warehouse_note WHERE Order_ID=%d AND Is_Alert='Y' AND Warehouse_ID=%d", mysql_real_escape_string($order->ID), mysql_real_escape_string($session->Warehouse->ID)));
while($data->Row){
