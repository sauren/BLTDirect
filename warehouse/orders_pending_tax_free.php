...
<div id="PageWrapper">
	<div id="Page">
		...
		<h1>Products to Despatch (Tax Free)</h1><br />
        <?php
		...
		if($session->Warehouse->Type == 'S') {
			...
			$sqlWhere .= sprintf("WHERE o.Is_Declined='N' AND o.Is_Warehouse_Declined='N' AND (o.Status LIKE 'Packing' OR o.Status LIKE 'Partially Despatched') AND o.Is_Collection='N' ");
			...
			$sqlWhere .= sprintf("WHERE ((w.Type='B' AND w.Warehouse_ID=%d) OR (w.Type='S' AND s.Is_Drop_Shipper='N' AND s.Drop_Shipper_ID=%d)) AND o.Is_Declined='N' AND o.Is_Warehouse_Declined='N' AND o.Total>0 AND o.TotalTax=0 AND o.Is_Collection='N' ", $session->Warehouse->ID, $session->Warehouse->Contact->ID);
			$sqlGroup .= sprintf("GROUP BY o.Order_ID ");

            switch($form->GetValue('status')) {
                case 'N':
					$sqlWhere .= "AND (o.Status LIKE 'Partially Despatched' OR o.Status LIKE 'Packing') ";
                case 'PS':
					$sqlWhere .= "AND (o.Status LIKE 'Partially Despatched' OR o.Status LIKE 'Packing') AND ol.Line_Status<>'Backordered' ";
					break;
                case 'BO':
                	$sqlWhere .= "AND (ol.Line_Status LIKE 'Backordered' OR o.Backordered='Y') AND o.Status NOT LIKE 'Despatched' AND o.Status NOT LIKE 'Cancelled' ";
					break;
			}
		}
