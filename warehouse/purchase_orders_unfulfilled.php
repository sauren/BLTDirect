...
<div id="PageWrapper">
	<div id="Page">
		<div id="PageContent"><!-- InstanceBeginEditable name="pageContent" -->
			<h1>Unfulfilled Purchase Orders</h1>
			<br />

			<?php
			$table = new DataTable("orders");
			$table->SetSQL(sprintf("SELECT p.* FROM purchase AS p WHERE p.Supplier_ID=%d AND p.For_Branch>0 AND (p.Purchase_Status LIKE 'Unfulfilled' OR p.Purchase_Status LIKE 'Partially Fulfilled') AND Purchased_On<NOW()", $supplierId));
			$table->AddField('Purchase Order ID', 'Purchase_ID', 'left');
			$table->AddField('Purchase Order Date', 'Purchased_On', 'left');
			...
			$table->AddLink("purchase_order_details.php?id=%s", "<img src=\"images/folderopen.gif\" alt=\"Open Purchase Order Details\" border=\"0\">", "Purchase_ID");
			$table->SetMaxRows(25);
