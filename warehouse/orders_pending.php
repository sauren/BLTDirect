...
<div id="PageWrapper">
	<div id="Page">
        ...
        $form->AddOption('status','PS','View Orders to Pack or Partially Shipped');
        $form->AddOption('status','BO','View Back Orders');
		...
		<tr>
			<td class="label"><?php echo $form->GetLabel('status'); ?></td>
			<td class="input">
				<?php echo $form->GetHTML('status'); ?>
				<input type="submit" name="search" value="Search" class="submit">
			</td>
			<td rowspan="3">
				<h3>Legend</h3><br />

				<div style="padding: 2px 0 2px 0;"><img src="images/legend_red.gif" height="22" width="22" alt="VAT Free" align="absmiddle" /> VAT free orders.</div>
				<div style="padding: 2px 0 2px 0;"><img src="images/legend_orange.gif" height="22" width="22" alt="Restocked" align="absmiddle" /> Restocked undeclined warehouse orders.</div>
				<div style="padding: 2px 0 2px 0;"><img src="images/legend_yellow.gif" height="22" width="22" alt="Next Day Delivery" align="absmiddle" /> Next day delivery orders.</div>
				<div style="padding: 2px 0 2px 0;"><img src="images/legend_green.gif" height="22" width="22" alt="Undeclined" align="absmiddle" /> Undeclined warehouse orders.</div>
				<div style="padding: 2px 0 2px 0;"><img src="images/legend_cyan.gif" height="22" width="22" alt="Over &pound;100.00" align="absmiddle" /> Value over &pound;100.00 orders.</div>
				<div style="padding: 2px 0 2px 0;"><img src="images/legend_blue.gif" height="22" width="22" alt="Plain Label" align="absmiddle" /> Plain despatch label orders.</div>
				<div style="padding: 2px 0 2px 0;"><img src="images/legend_purple.gif" height="22" width="22" alt="Absent Stock Profile" align="absmiddle" /> Contains absent stock for products in profile.</div>
			</td>
		</tr>
		<tr>
			<td></td>
			<td class="input">
				<input name="print orders" type="button" id="Print Orders" value="Print Orders Together" class="greySubmit" onclick="popUrl('print_picking.php?style=break&status=<?php print $form->GetValue('status'); ?>',800, 600);" />
				<input name="print orders" type="button" id="Print Orders" value="Print Orders Individually" class="greySubmit" onclick="popUrl('print_picking.php?style=page&status=<?php print $form->GetValue('status'); ?>',800, 600);" />
			</td>
		</tr>
		<tr>
			<td></td>
			<td class="input">
				<input name="purchaseDate" id="purchaseDate" type="text" value="<?php print date('d/m/Y', mktime(0, 0, 0, date('m'), date('d')-1, date('Y'))); ?>" onclick="scwShow(this,this);" onfocus="scwShow(this,this);" />
				<input name="print purchases" type="button" id="Print Purchases" value="Print Purchase Orders" class="greySubmit" onclick="popUrl('print_purchases.php?date=' + document.getElementById('purchaseDate').value, 800, 600);" />
			</td>
		</tr>
		...
		if($session->Warehouse->Type == 'S') {
			...
			$sqlWhere .= sprintf("WHERE o.Is_Declined='N' AND o.Is_Failed='N' AND o.Is_Warehouse_Declined='N' AND (o.Status LIKE 'Packing' OR o.Status LIKE 'Partially Despatched') AND o.Is_Collection='N' ");
			$sqlGroup .= sprintf("GROUP BY o.Order_ID ");

			switch($form->GetValue('status')) {
                case 'PS':
					$sqlWhere .= "AND ol2.Order_Line_ID IS NULL ";
					break;
                case 'BO':
                	$sqlWhere .= "AND ol2.Order_Line_ID IS NOT NULL ";
					break;
			}
		} else {
			...
			$sqlWhere .= sprintf("WHERE ((w.Type='B' AND w.Warehouse_ID=%d) OR (w.Type='S' AND s.Is_Drop_Shipper='N' AND s.Drop_Shipper_ID=%d)) AND ((o.Total=0) OR (o.Total>0 AND o.TotalTax>0)) AND o.Is_Collection='N' ", $session->Warehouse->ID, $session->Warehouse->Contact->ID);
			$sqlGroup .= sprintf("GROUP BY o.Order_ID ");

            switch($form->GetValue('status')) {
                case 'N':
					$sqlWhere .= "AND (o.Status LIKE 'Partially Despatched' OR o.Status LIKE 'Packing') AND o.Is_Declined='N' AND o.Is_Failed='N' AND o.Is_Warehouse_Declined='N' ";
                case 'PS':
					$sqlWhere .= "AND (o.Status LIKE 'Packing' OR o.Status LIKE 'Partially Despatched') AND ol.Line_Status<>'Backordered' AND o.Is_Declined='N' AND o.Is_Failed='N' AND o.Is_Warehouse_Declined='N' ";
					break;
                case 'BO':
                	$sqlWhere .= "AND (ol.Line_Status LIKE 'Backordered' OR o.Backordered='Y') AND o.Status NOT LIKE 'Despatched' AND o.Status NOT LIKE 'Cancelled' AND o.Is_Declined='N' AND o.Is_Failed='N' AND o.Is_Warehouse_Declined='N' ";
					break;
			}
		}
