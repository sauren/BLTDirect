...
$foundSup = false;
if(strtoupper($session->Warehouse->Type) == 'S' ){
	$supFinder = new DataQuery(sprintf("SELECT * FROM supplier_product WHERE Supplier_ID = %d AND Product_ID = %d",mysql_real_escape_string($session->Warehouse->Contact->ID),mysql_real_escape_string($warehouseStock->Product->ID)));
...
if(isset($_REQUEST['confirm']) && strtolower($_REQUEST['confirm']) == "true"){
	...
	if($foundSup) $supplierProd->SKU = $form->GetValue('sku');
	if($form->Valid){
		if($foundSup) $supplierProd->Update();
		$warehouseStock->Update();
		redirect("Location: products_stocked.php");
	}
}
?>
...
<div id="PageWrapper">
	<div id="Page">
		...
		<br>
<?php
if($warehouseStock->Warehouse->ID == $session->Warehouse->ID){
// Show Error Report if Form Object validation fails
if(!$form->Valid){
	echo $form->GetError();
	echo "<br>";
}
$window = new StandardWindow("Edit an item of stock.");
...
echo $webForm->AddRow("&nbsp;", sprintf('<input type="button" name="back" value="Back" class="greySubmit" onClick="window.self.location=\'products_stocked.php\';"> <input type="submit" name="update" value="Update" class="submit" tabindex="%s">', $form->GetTabIndex()));
echo $webForm->Close();
...
echo "<br>";
	...
	echo "Sorry, This product is stored under a different warehouse.";
}
?>
		<!-- InstanceEndEditable -->
		</div>
