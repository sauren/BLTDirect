...
<head>
	...
	<title>Lamp Base Examples</title>
	<!-- InstanceEndEditable -->
            ...
            <div id="Page">
                <div id="PageContent">
					...
					$data = new DataQuery(sprintf("SELECT * FROM article WHERE Article_Category_ID=%d AND Is_Active='Y' ORDER BY Created_On DESC", LAMP_BASE_ARTICLE_CAT));
						...
						while($data->Row) {
							print sprintf('<h1>%s</h1><p>%s</p>', $data->Row['Article_Title'], nl2br(stripslashes($data->Row['Article_Description'])));
							$data->Next();
						}
					...
					while($data->Row) {
						...
						<div class="baseExample">
							<?php
							if(!empty($data->Row['Image']) && file_exists($GLOBALS['BASE_IMAGES_DIR_FS'].$data->Row['Image'])) {
								echo sprintf('<a href="search.php?filter=%d"><img src="%s%s" alt="%s" width="100" height="100" border="0" /></a>', $data->Row['Specification_Value_ID'], $GLOBALS['BASE_IMAGES_DIR_WS'], $data->Row['Image'], $data->Row['Name']);
							}
							?>

							<span class="content">
								<p><strong><?php echo $data->Row['Name']; ?></strong><br />View light bulbs with a <?php echo $data->Row['Value']; ?> lamp base.</p>
								<p class="search"><a href="search.php?filter=<?php echo $data->Row['Specification_Value_ID']; ?>" title="Search for <?php echo $data->Row['Name']; ?> Base Light Bulbs">Search</a></p>
							</span>
						</div>
