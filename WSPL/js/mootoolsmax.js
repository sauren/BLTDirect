...
* Deveus(R)
* Modified for Evance:
	* initialize methods within classes is now init
	* Added new Elements.getElementsByAttribute() method
*/
...
Class: Class
	...
	Creates a new class, its init method will fire upon class instantiation.
...
var Class = function(properties){
	var klass = function(){
		return (arguments[0] !== null && this.init && $type(this.init) == 'function') ? this.init.apply(this, arguments) : this;
	};
	$extend(klass, this);
	...
	Example:
		...
		var Cat = Animal.extend({
			init: function(name, age){
				this.parent(age); //will call the previous init;
				this.name = name;
			}
		});
...
Element.extend({
	...
	*/
	...
	Example:
		...
		<img src="../../dev/old/whatever.gif" alt="whatever dude">
		(end)
	*/
		...
		if(this.childNodes.length && this.childNodes.length > 0){
			for(var i=0; i < this.childNodes.length; i++){
				if(this.childNodes[i] && this.childNodes[i].nodeType == 1){
					if(this.childNodes[i].getAttribute(str)){
	...
	add: function(fn){
		...
		if (document.readyState && window.webkit){
			...
			if (!$('ie_ready')){
				...
				document.write('<script id="ie_ready" defer src="../../dev/old/' + src + '"><\/script>');
