...
$form->AddField('Order_ItemName', '', 'hidden', '', 'anything', 1, 255, false);
...
if(!empty($message)) {
	...
	$enquiryLine->Message = sprintf("Enquiry from bltdirect.co.uk\n\n%s", $message);
	$enquiryLine->Add();
}

redirect(sprintf("Location: http://www.bltdirect.co.uk/enquiry.php?status=sent&Order_ItemName=%s", $form->GetValue('Order_ItemName')));