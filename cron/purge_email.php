...
$script = 'Purge Email Queue';
$fileName = 'purge_email.php';

## BEGIN SCRIPT
new DataQuery(sprintf("DELETE FROM eq USING email_queue AS eq INNER JOIN email_queue_module AS eqm ON eq.Module_ID=eqm.Email_Queue_Module_ID WHERE eqm.Is_Purging_Active='Y' AND eq.Created_On<ADDDATE(NOW(), INTERVAL -1 MONTH)"));
new DataQuery(sprintf("OPTIMIZE TABLE email_queue"));
## END SCRIPT
