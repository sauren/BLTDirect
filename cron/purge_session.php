...
$script = 'Purge Customer Sessions';
$fileName = 'purge_session.php';

## BEGIN SCRIPT
new DataQuery(sprintf("DELETE FROM customer_session WHERE Created_On<ADDDATE(NOW(), INTERVAL -1 WEEK)"));
new DataQuery(sprintf("DELETE FROM customer_session_item WHERE Created_On<ADDDATE(NOW(), INTERVAL -1 WEEK)"));

new DataQuery(sprintf("OPTIMIZE TABLE customer_session"));
new DataQuery(sprintf("OPTIMIZE TABLE customer_session_item"));
## END SCRIPT
