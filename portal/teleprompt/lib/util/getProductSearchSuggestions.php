...
if(!isset($_REQUEST['criteria'])) {
	...
	$data = new DataQuery(sprintf("SELECT Product_ID, Product_Title, SKU FROM product WHERE %s AND Discontinued='N' AND Is_Demo_Product='N' ORDER BY Product_Title ASC LIMIT 0, %d", implode(' AND ', $items), mysql_real_escape_string($limit)));
	while($data->Row) {
	...
	if($recentCount > 0) {
		...
		$data = new DataQuery(sprintf("SELECT Product_ID, Product_Title, SKU FROM product WHERE (%s) AND Discontinued='N' AND Is_Demo_Product='N' ORDER BY Product_Title ASC LIMIT 0, %d", implode(' OR ', $items), mysql_real_escape_string($limit)));
		while($data->Row) {
			$products[] = array(strip_tags($data->Row['Product_Title']), $data->Row['Product_ID'], $data->Row['SKU']);
