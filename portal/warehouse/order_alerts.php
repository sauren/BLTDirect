...
$data = new DataQuery(sprintf("SELECT Order_Note_ID FROM order_note WHERE Order_ID=%d AND Is_Alert='Y'", mysql_real_escape_string($order->ID)));
while($data->Row){
...
$data = new DataQuery(sprintf("SELECT Order_Warehouse_Note_ID FROM order_warehouse_note WHERE Order_ID=%d AND Is_Alert='Y'", mysql_real_escape_string($order->ID)));
while($data->Row){
...
if($action == 'dismiss') {
	foreach($customerNotes as $note) {
		...
		if(isset($_REQUEST[$id]) && ($_REQUEST[$id] == 'Y')){
			$data = new DataQuery(sprintf("UPDATE order_note SET Is_Alert='N' WHERE Order_Note_ID=%d", mysql_real_escape_string($note->ID)));
			$data->Disconnect();
		}
	}
	...
	foreach($warehouseNotes as $note) {
		...
		if(isset($_REQUEST[$id]) && ($_REQUEST[$id] == 'Y')){
			$data = new DataQuery(sprintf("UPDATE order_warehouse_note SET Is_Alert='N' WHERE Order_Warehouse_Note_ID=%d", mysql_real_escape_string($note->ID)));
			$data->Disconnect();
		}
	}
