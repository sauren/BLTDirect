...
$data = new DataQuery(sprintf("SELECT Timesheet_ID, Type, Date, Hours FROM timesheet WHERE User_ID=%d AND Date>='%s' AND Date<'%s' ORDER BY Date ASC", $GLOBALS['SESSION_USER_ID'], mysql_real_escape_string($start), mysql_real_escape_string($end)));
while($data->Row) {
	$object = new stdClass();
        ...
        foreach($objects as $objectItem) {
			if($editThreshold <= strtotime($objectItem->Date)) {
				...
				if($hours == 0) {
					...
					new DataQuery(sprintf("UPDATE timesheet SET Hours=%f WHERE Timesheet_ID=%d", mysql_real_escape_string($hours), mysql_real_escape_string($objectItem->Data['Timesheet_ID'])));
				}
			}
		}

		redirectTo(sprintf('?offset=%d&year=%d', $m, $y));
	}
}
