...
echo '<tr><td>&nbsp</td><td><input name="Print orders" type="button" id="Print Orders" value="Print Orders Together" class="btn" onclick="popUrl(\'print_picking.php?style=break&status='.$form->GetValue('status').'&bid='.$warehouseId.'\',800, 600)"><input name="Print orders" type="button" id="Print Orders" value="Print Orders Individually" class="btn" onclick="popUrl(\'print_picking.php?style=page&status='.$form->GetValue('status').'&bid='.$warehouseId.'\',800, 600)"></td></tr>';
echo $webForm->Close();
...
$sqlFrom = sprintf('FROM warehouse AS w INNER JOIN order_line AS ol ON ol.Despatch_From_ID=w.Warehouse_ID INNER JOIN orders AS o ON o.Order_ID=ol.Order_ID LEFT JOIN postage AS pg ON o.Postage_ID=pg.Postage_ID ');
$sqlWhere = sprintf('WHERE w.Type_Reference_ID=%d AND ol.Despatch_ID=0 AND o.Total>0 AND o.TotalTax=0 AND o.Is_Declined=\'N\' AND o.Is_Failed=\'N\' AND o.Is_Warehouse_Declined=\'N\' AND o.Is_Awaiting_Customer=\'N\' AND o.Is_Collection=\'N\' ', mysql_real_escape_string($branchID));
$sqlGroup = sprintf('GROUP BY o.Order_ID ');

if($form->GetValue('status') == 'N') {
	$sqlWhere .= sprintf('AND Status IN (\'Partially Despatched\', \'Packing\') ');
	
} elseif($form->GetValue('status') == 'PK') {
	$sqlWhere .= sprintf('AND Status IN (\'Packing\') ');
				
} elseif($form->GetValue('status') == 'PD') {
	$sqlWhere .= sprintf('AND Status IN (\'Partially Despatched\') ');
