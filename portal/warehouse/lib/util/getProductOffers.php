...
$data = new DataQuery(sprintf("SELECT po.Product_ID, pi.Image_Src FROM product_offers AS po INNER JOIN product AS p ON p.Product_ID=po.Product_ID LEFT JOIN product_images AS pi ON pi.Product_ID=p.Product_ID AND pi.Is_Active='N' AND pi.Is_Primary='N' AND pi.Image_Title LIKE 'specialoffer' WHERE p.Is_Active='Y' AND p.Discontinued='N' AND ((NOW() BETWEEN p.Sales_Start AND p.Sales_End) OR (p.Sales_Start='0000-00-00 00:00:00' AND p.Sales_End='000-00-00 00:00:00') OR (p.Sales_Start='0000-00-00 00:00:00' AND p.Sales_End>NOW()) OR (p.Sales_Start<=NOW() AND p.Sales_End='0000-00-00 00:00:00')) AND ((NOW() BETWEEN po.Offer_Start_On AND po.Offer_End_On) OR (po.Offer_Start_On='0000-00-00 00:00:00' AND po.Offer_End_On='000-00-00 00:00:00') OR (po.Offer_Start_On='0000-00-00 00:00:00' AND po.Offer_End_On>NOW()) OR (po.Offer_Start_On<=NOW() AND po.Offer_End_On='0000-00-00 00:00:00')) GROUP BY po.Product_ID"));
while($data->Row) {
